...
| 方面 | Core 目标 |
|---|---|
| 数值参考 T1 | 至少三个不同机制家族，优先 F3、F4、F1，F2作为替补 |
...
| A4：F1／F2 | F1 根因修复；F2 静态与动态组件 | 第三家族及替补资格、生产批次 | 可立即取证 |
...
F1优先完成运行域问题，并检查真正的实体边界和原生排除。切换边界方案时必须验证法向、ghost 和完整配置，不能仅改变 CFL 就声称完成边界修复。
