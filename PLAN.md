...
- 2026-09-26 UPDATE-214：V17 主机差分诊断与 Linux v6.8 上游源码核对发现，`FAN_REPORT_PIDFD` 属于 `FANOTIFY_ADMIN_INIT_FLAGS`，且 v6.8 `capable(CAP_SYS_ADMIN)` 检查 `init_user_ns`；因此 V17 的 permission 与 name/FID 两个 exact group 都需要初始 user namespace 中的 `CAP_SYS_ADMIN`。在 `/tmp` 与工作区 `/home` 两个独立 ext4 mount 上，临时 `AT_HANDLE_FID` 调用都成功；FID/name flags `0x1e03` 成功，加入 PIDFD 的 exact `0x1e83` 与 permission exact `0x0087` 均为 `EPERM`。新增 [V18 additive proposal](lagrangian-fluid-lab/reports/F8-R008-TERMINAL-COMPLETION-EVIDENCE-CONTRACT-V18-2026-09-26.zh-CN.md) 修正前置条件、user namespace 语义和 exact 参数记录，保持 PIDFD actor join；`event_f_flags=0x80000` 明确限定为本机 x86-64 glibc ABI，不跨架构移植。GPT-6 Luna Max 配置 subagent 首轮与 follow-up 均认为核心结论准确、无 P0/P1；P3 参数映射建议已并入，先前 P2 因混淆 V17 历史说明而撤回。reviewer identity 未 attested。`sudo -n` 需密码；未做特权探测或 pinned-kernel conformance，无 implementation/runtime/qualification/credit 变化。详见 [UPDATE-214](lagrangian-fluid-lab/reports/CORE-CONTINUATION-STATUS-2026-09-26-UPDATE-214.zh-CN.md)。
