...
from scripts import f3_material_row30_resource_preflight_v5 as preflight_v5


def test_v5_binds_v4_history_and_preserves_no_worker_scope(monkeypatch) -> None:
    ...
    value = preflight_v5.build(environment_probe=lambda: {}, process_probe=lambda: [])

    assert value["schema"] == preflight_v5.SCHEMA
    assert value["record_id"] == preflight_v5.RECORD_ID
    assert value["authorized_scope"] == preflight_v5.AUTHORITY
    assert value["supersedes"]["path"].endswith("resource-preflight-v4/receipt.json")
    assert value["worker_launch_authorized"] is False
    assert value["execution_controls"]["material_worker_started"] is False
    assert all("v5" in item["path"] for item in value["bindings"])


def test_v5_write_once_records_lock_without_worker_authority(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(preflight_v5, "build", lambda **kwargs: {
        "schema": preflight_v5.SCHEMA,
        "record_id": preflight_v5.RECORD_ID,
        "status": "blocked_no_worker_authorized",
        "worker_launch_authorized": False,
    })
    scope = tmp_path / "row30-v5"

    value = preflight_v5.write_once(scope_dir=scope)
    ...
    with pytest.raises(FileExistsError, match="namespace already exists"):
        preflight_v5.write_once(scope_dir=scope)


def test_v5_failure_preserves_consumed_lock_and_terminal_failure(tmp_path: Path, monkeypatch) -> None:
    def fail(**kwargs):
        raise RuntimeError("synthetic fail-closed preflight error")

    monkeypatch.setattr(preflight_v5, "build", fail)
    scope = tmp_path / "row30-v5-failure"
