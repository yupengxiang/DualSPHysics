...
from scripts import f3_material_row30_resource_preflight_v4 as preflight_v4


def test_v4_binds_v3_history_and_preserves_no_worker_scope(monkeypatch) -> None:
    ...
    value = preflight_v4.build(environment_probe=lambda: {}, process_probe=lambda: [])

    assert value["schema"] == preflight_v4.SCHEMA
    assert value["record_id"] == preflight_v4.RECORD_ID
    assert value["authorized_scope"] == preflight_v4.AUTHORITY
    assert value["supersedes"]["path"].endswith("resource-preflight-v3/receipt.json")
    assert value["worker_launch_authorized"] is False
    assert value["execution_controls"]["material_worker_started"] is False
    assert all("v4" in item["path"] for item in value["bindings"])


def test_v4_write_once_records_lock_without_worker_authority(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(preflight_v4, "build", lambda **kwargs: {
        "schema": preflight_v4.SCHEMA,
        "record_id": preflight_v4.RECORD_ID,
        "status": "blocked_no_worker_authorized",
        "worker_launch_authorized": False,
    })
    scope = tmp_path / "row30-v4"

    value = preflight_v4.write_once(scope_dir=scope)
    ...
    with pytest.raises(FileExistsError, match="namespace already exists"):
        preflight_v4.write_once(scope_dir=scope)


def test_v4_failure_preserves_consumed_lock_and_terminal_failure(tmp_path: Path, monkeypatch) -> None:
    def fail(**kwargs):
        raise RuntimeError("synthetic fail-closed preflight error")

    monkeypatch.setattr(preflight_v4, "build", fail)
    scope = tmp_path / "row30-v4-failure"
