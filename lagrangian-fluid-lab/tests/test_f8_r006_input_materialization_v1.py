...
from scripts import f8_r006_input_materialization_v1 as materializer
...
def test_materialized_inputs_match_the_immutable_static_precommit() -> None:
...
def test_definition_proof_covers_four_boundary_layers_interface_and_domain() -> None:
    definition = design.definition_xml(design.parameters())
    proof = materializer.prove_definition(definition)
    assert proof["domain_z_m"] == pytest.approx([-0.0825, 0.0825])
...
def test_r006_control_remains_byte_identical_to_retained_positive_control() -> None:
    control = design.acceleration_csv(design.parameters()).encode("utf-8")
    assert control == (LAB / design.R005_CONTROL).read_bytes()
...
def test_review_hash_mismatch_is_rejected_before_materialization() -> None:
    ...
    control = design.acceleration_csv(design.parameters()).encode("utf-8")
    review = materializer.load_json(materializer.REVIEW)
    review["precommitted_input_bytes"]["definition_sha256"] = "0" * 64
...
def test_materialization_receipt_is_static_only_and_hash_closed() -> None:
    ...
    assert value["status"] == "one_time_r006_inputs_materialized_static_only"
