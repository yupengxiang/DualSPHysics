...
检索源实际贡献为出版方／作者官方页面：PMLR、NeurIPS proceedings、arXiv、AAAI proceedings 与 FluidLab 作者项目页。未发现本机 Zotero/Obsidian 连接器、本地 `papers/` 或 `literature/` PDF 库，也未发现 `verify_papers.py` 或相应 ARIS helper。因此按 research-lit 的降级规则，每项均保留自动状态 `UNVERIFIED`；这里的人工核对不冒充 arXiv／Crossref／Semantic Scholar 三源自动校验。
