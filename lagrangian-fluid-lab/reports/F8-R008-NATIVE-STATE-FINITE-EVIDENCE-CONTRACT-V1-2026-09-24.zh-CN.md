...
状态：实现对齐修订版。此前 Terra High（`gpt-5.6-terra`, high；agent `01a0d376-e477-7ab0-be6e-55325dc9de10`）的 `PASS` 仅覆盖修订前的静态设计，不能视为本修订或实现的审查结论；待 Terra High 复审。本方案只定义对已闭合 B/C/D + table-v2 结果生成的逐帧有限值证据；不实现 native-integrity adjudicator，不给 T1 或资格信用。
...
实现必须在单次收集中持有 B/C/D bundle-root 与各 stage output root 的目录 FD。现有、已冻结的 v1 stage verifier 只接受路径且其源哈希已被既有 D 授权/审查材料绑定，因此编排层调用该入口后必须将 verifier 返回的 root/output `(device,inode)` 与 held FD 身份逐一比对；table-chain verifier 也必须以相同绝对路径完成完整重验。这里的保证是每个校验边界均指向同一目录对象，并非底层 verifier 直接共用同一 FD 或文件系统原子快照；不得把该限制描述成零 TOCTOU。持有 table FD 至结束。每帧 C 文件均在 held C output FD 下 no-follow 打开，验证 single-link regular-file、声明的 size/SHA，扫描前后重核 descriptor 身份与 SHA。对 finite 扫描实际使用的同一 raw `ScanResult`，还须重新读取对应 D safe-decode receipt，并以该 `ScanResult` 逐项重算/比对所有数组描述符、数组字节 SHA、确定性 XML、安全解码数组及文件/目录清单；不能只依赖另一次 table-chain 检查。开始与结束时均须验证 B/C/D 三个 stage receipt 的 `status == "passed"`、重新校验完整 provenance-chain 和 table-v2 semantic chain，并从 held roots 重读、核对参与绑定的 B/C/D receipts、manifests、B cohort/input 文件、C frames、D safe-decode outputs 和 table 的内容绑定、字节数与 SHA；C frames 与 table 还须比较 held descriptor 身份。任一状态非 `passed`、绑定字节变化、相关 held FD 身份变化或复核失败即 fail-closed，不进入 evidence 发布阶段。顶层 bindings 必须对应 post-scan 重新验证仍未变化的字节；对目录项或非持有的输入文件，本双侧复验不是可证明的原子快照，残余并发改写竞态仍属外部信任边界。

输出须绑定本次校验所用的 B/C/D authorization SHA-256、独立 code-review receipt SHA-256、table/schema review receipt SHA-256，以及 runtime assumption 的规范化摘要。当前 table-chain 使用 metric-free 的 table-v2 semantic verifier，因此不读取或声称绑定 metric-review receipt。上述摘要只标识调用方提供的信任上下文；不表示授权签名真实性、review 来源真实性、loaded-module identity 或 runtime 已被独立认证。
...
新输出独立于冻结 D v1 bundle，不能向其 `outputs/` 或 manifests 追加文件。必须持有 caller-owned、非 group/world-writable 的独立输出目录 FD，并在发布前重开请求路径核对目录对象身份；使用该目录上的 unnamed `O_TMPFILE` 普通文件（初始 link count 为 0），写入后 fsync 并从 held FD 重算大小/SHA，再通过 procfs FD hard-link 做原子 no-replace 发布；既有目标不得覆盖，不支持该 unnamed/no-replace 发布能力时 fail-closed。原子 link 是提交点：之后的 final-name inode/hash 检查、目录 fsync、FD close 和请求目录路径重绑定状态都必须明确作为发布结果字段返回；提交后错误不得伪装成“未发布”，也不得删除可能已被并发替换的 final name。调用方遇到 post-commit 检查或 durability 状态失败时必须按“已提交但未完全确认”处理，不能自动重试同一 case 路径。下面代码块列出固定常量字段，不是完整 JSON 样例：
...
正负例仅用临时合成 BI4。必须证明：fluid-only table-v2 在 non-fluid state 含 NaN/Inf 时仍可能语义通过，而本 evidence output 会定位并报告相应 raw state 非有限值；Pos float32 与 Posd float64 均保留原生 dtype/位模式；D safe-decode array manifest 与 finite 扫描实际使用的 C raw descriptor 不一致时 fail-closed 且不发布；raw FD 在扫描中被截断/替换时 evidence collection 拒绝且不发布；不支持的数组类型、错误 count、重复/缺失 required array 均拒绝；全数组无有限值时 extrema 和 ordinal 为 JSON null；超出 frozen 粒子上限拒绝。
