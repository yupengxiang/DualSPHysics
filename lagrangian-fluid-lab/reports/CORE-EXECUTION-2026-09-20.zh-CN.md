...
## 本轮追加：F5 几何修复 v3/v4 CPU/native 硬门失败与路线收口

- v3 独立 root receipt [root-review-receipt-v1.json](../campaigns/core-v1/cfd/f5-wave-runup-third-t1/geometry-repair-audit-v1/root-review-receipt-v1.json) SHA-256 为 `24b1bf0780dc905a7e17e31aeac7be1a2e98b5b0f9bd49eccefcff6e4e6b9dc1`。它绑定官方 GenCase 模板中的 `setmkvoid`／`drawfilestl autofill=true` 语法和新的 v3 输入身份，只授权一次 CPU GenCase/native decode；solver、GPU、queue、ledger、registry、matrix 均关闭。
- v3 物化时发现一个没有执行过的静态实现错误：void precursor 后未显式恢复 `setmkbound` marker。该错误的 Definition 与合同以 [materialization-audit-v1](../campaigns/core-v1/cfd/f5-wave-runup-third-t1/geometry-repair-audit-v1/materialization-audit-v1/invalid-materialization-audit-v1.json) 保留，明确 `gencase/native/solver/GPU/queue/ledger/registry/matrix=0`；修正后才使用同一 root-reviewed v3 路径执行唯一预检。
- v3 预检 [preflight.json](../campaigns/core-v1/cfd/f5-wave-runup-third-t1/geometry-repair-audit-v1/preflight-v3/preflight.json) SHA-256 为 `eb91770a8a52b1495438d2ff9fe13d265d05053cb15109f3251a9da6ace25441`。GenCase 返回 `0`，生成 `1,374,477` 个粒子（fluid `1,279,855`），native ID、有限值、质量 metadata 和静态包络均通过；但 fluid-only frame 0 在变换后的 block STL 内仍有 **1** 个粒子，Slope 为 `0`。因此 `frame0_fluid_endpoint_inside_blocks=false`，状态为 `cpu_native_preflight_failed_geometry_repair_gate`，`qualified=false`、`matrix_credit=0`，没有 solver/GPU/queue。
- 该单粒子坐标为 `[5.94000006, 0.105, 0.09]`；它不是 boundary 粒子统计造成的假阳性。v3 fluid-only gate 使用 `fluid_first_id=94622`、`1,279,855` 个 fluid 粒子和固定 `1e-8 m` 判定容差，失败行不能被接触带解释或删除。
- v4 独立 root receipt [autofill-bound-root-review-v1.json](../campaigns/core-v1/cfd/f5-wave-runup-third-t1/geometry-repair-audit-v1/autofill-bound-root-review-v1.json) SHA-256 为 `c9f0a76ecee5876d42c0d49eda50f9cc4e55d371e9f44ffa1dd70b75d33ffb87`。它根据 v3 的实际失败，采用官方 `CaseCurrentWheel_Def.xml` 已出现的 `setmkbound` 后单次 `drawfilestl autofill=true` 作为第二个独立假设，换用全新的 v4 Definition、资产和 preflight output stem；仍只授权一次 CPU/native 预检。
- v4 预检 [preflight.json](../campaigns/core-v1/cfd/f5-wave-runup-third-t1/geometry-repair-audit-v1/preflight-v4/preflight.json) SHA-256 为 `75f55fd80d01a067acb72e22888797b14e30ce22fc22d98ac90838c9ef519c2d`。即使使用官方 autofill-bound 物化，生成／native 粒子计数和质量门仍通过，fluid-only frame 0 仍得到同一个 block 内粒子 `1`；因此 v4 也严格失败，`matrix_credit=0`，未启动 solver/GPU。
- v3/v4 的合并失败摘要 [failure-summary-v1.json](../campaigns/core-v1/cfd/f5-wave-runup-third-t1/geometry-repair-audit-v1/failure-summary-v1.json) SHA-256 为 `d18299fd1335b9c65a33839256adbf26f165dfb61fe8636aaf9712ccc9282336`。当前 F5 geometry repair 线按“两类有证据修复假设各一次”的规则收口；v3/v4 输入不重跑、不提交 anchor、不进入 T1 matrix。若以后重新做 F5，必须另立 root-level hypothesis 和新 identity；当前第三家族转由独立 F2 路线审计。
- 新增 v3/v4 root-review、writer、preflight 和失败审计相关回归共 **15 passed**，所有执行控制保持 zero credit；这些产物只增加失败证据，不改变 Core registry。Core gate 仍为 T1 `F3/F4`、宏观 T2 `0/2`、formal training `0/9`。

## 本轮追加：F2 静态接收盆自由落体捕获路线提案审计

- 独立路线审计 [F2-STATIC-RECEIVER-BALLISTIC-CATCH-ROOT-REVIEW-PROPOSAL-V1-2026-09-21.zh-CN.md](F2-STATIC-RECEIVER-BALLISTIC-CATCH-ROOT-REVIEW-PROPOSAL-V1-2026-09-21.zh-CN.md) SHA-256 为 `4d42f68e1c7b91a375cdfe11571f088331adeebfd4df3f72fb225a709efe65fe`。候选 scope 为 `F2_static_receiver_ballistic_catch_x_v1`，机制是固定外槽、固定 raised open-top 接液盆和自由下落有限液团，观察接触后的保留／溢出分配；它与 moving-cup、receiver/weir、static-hold 和 submerged-orifice v2/v3/v4 物理独立。
- 提案 [f2-static-receiver-ballistic-catch-proposal-v1.json](../campaigns/core-v1/cfd/f2-static-receiver-ballistic-catch-proposal-v1.json) SHA-256 为 `e2f69380cdecf5952ec8d7d2d69aec43481d421b245e212395df42aa47ed28c9`，固定 15 行分母（13 空间＋2 时间／输出对照），当前 `executed=0`、`credit=0`、`qualification_claim=none`、`matrix_credit=0`。旧 airborne-slug 证据只用于有限弹道／转移可执行性判断，不继承旧 Definition、generated/native 输出、粒子身份或资格。
- 只读校验实现 [f2_static_receiver_ballistic_catch_proposal_v1.py](../scripts/f2_static_receiver_ballistic_catch_proposal_v1.py) SHA-256 为 `3cd20870daa5a01ffa2a37e99985b538ab93747f70ac291d7206bbe2d3e919f6`；结构测试 [test_f2_static_receiver_ballistic_catch_proposal_v1.py](../tests/test_f2_static_receiver_ballistic_catch_proposal_v1.py) SHA-256 为 `a63f6346344a57c3e7d8f48001584c44f01cb2a101acca5c849d69639749f456`，定向回归 **5 passed**。本轮没有物化 Definition、调用 GenCase/native decoder/solver、启动 GPU／queue 或修改 ledger、registry、matrix。
- 该路线仍是 `proposal_only_root_review_required`，不能计入第三 T1 家族。后续必须由 root review 冻结 literal fresh Definition 和新 namespace，再单独授权一次 CPU/native preflight；Core gate 保持 T1 `F3/F4`、宏观 T2 `0/2`、formal training `0/9`，固定缺失分母不变。

## 本轮追加：F2 静态接收盆路线 root review 与 CPU/native 预检

- root-review receipt [root-review-receipt-v1.json](../campaigns/core-v1/cfd/f2-static-receiver-ballistic-catch-v1/root-review-receipt-v1.json) SHA-256 为 `3a4f61677f4e40df6882b317887a8f2d119ae1b8f919b561a614cc31678635c3`，状态为 `authorized_one_fresh_cpu_native_preflight_only`。它只授权一次 GenCase 和一次 native decode，solver、GPU、job、queue、ledger、registry、matrix 全部关闭。
- literal Definition [CORE_F2_STATIC_RECEIVER_BALLISTIC_CATCH_q0p50000000_dp0p007500000000_anchor_Def.xml](../campaigns/core-v1/cfd/f2-static-receiver-ballistic-catch-v1/input/CORE_F2_STATIC_RECEIVER_BALLISTIC_CATCH_q0p50000000_dp0p007500000000_anchor_Def.xml) SHA-256 为 `a84975730d456edc99ce5f6e6fe23703959b8e383fdefe5f1baeca579aec8ac8`；definition contract SHA-256 为 `94aadfe73168be67ff882f36b0bc2a8c5a78acf7e651dca041285e8cb08c14d4`。这是从文字几何契约新写的输入，没有复用旧 F2 XML 或生成产物。
- CPU/native preflight [preflight.json](../campaigns/core-v1/cfd/f2-static-receiver-ballistic-catch-v1/preflight/preflight.json) SHA-256 为 `2ad193aaadcd60a325dd18bea1f79635f3592671816afd673fde813ded2694cf`，状态为 `cpu_native_preflight_pass`，但 `qualified=false`、`qualification_claim=none`、`matrix_credit=0`。GenCase 返回 0；native 总粒子 `118,509`、边界 `100,077`、流体 `18,432`。ID 连续唯一、位置／速度／密度有限、外槽闭合面外端点 `0`、接液盆闭合面初始重叠 `0`，源质量相对误差 `2.22e-16`。
- writer、root review、preflight 的静态检查与回归共 **8 passed**，proposal validator 另有 **5 passed**。该预检只证明输入物化和初始硬门，未执行动力学事件窗、材料、15 行矩阵或 solver anchor；父 F2 15 行分母仍为 `0/15`，Core gate 仍为 T1 `F3/F4`、宏观 T2 `0/2`、formal training `0/9`。

## 本轮追加：F2 solver anchor 基础设施重试与科学硬门失败

- 第一次 solver anchor `f2-static-receiver-ballistic-catch-q05-anchor-v1` 在加载阶段因 worker 缺少官方 `LD_LIBRARY_PATH` 退出（返回码 `127`，找不到 `libdsphchrono.so`），没有生成 solver 帧。该执行器失败已保留，未计科学信用。
- 按一次有证据基础设施修复重试规则，infra-retry root review [infra-retry-root-review-v1.json](../campaigns/core-v1/cfd/f2-static-receiver-ballistic-catch-v1/solver-anchor-v1/infra-retry-root-review-v1.json) SHA-256 为 `b49579dab7616ea442c0d6bee4b242d3847d769bdd8848de9c0f4e8cd75f418e`，明确科学 Definition、GenCase XML/BI4、观察窗、阈值和分母均未变化，仍 `qualification_claim=none`、`matrix_credit=0`。
- 修复重试在 Ada 上正常完成 `1.5 s` 窗口，Run.out 为 `301` 帧（`0..300`）、`TimePart=0.005`、solver code 0。独立后审 [postrun-audit.json](../campaigns/core-v1/runtime/attempts/f2-static-receiver-ballistic-catch-q05-anchor-infra-retry-v1/20260921T111854-b642c199de99/product/postrun-audit.json) SHA-256 为 `c29f9afa7c0c4b4d0b2e18b11ba1151de5013fe1613b95fbb8b113149fc24ab3`；首末及固定抽样帧的 ID、有限值和外槽端点门通过，但 Run.out 报告 **64 个 excluded particles**（其中密度原因 44 个），约第120帧起流体粒子数由 `18,432` 变为 `18,368`。
- 因排除粒子硬门失败，后审状态为 `raw_solver_complete_scientific_hard_failure`、`hard_integrity_pass=false`，没有删除帧、填补粒子或继续做事件／材料解释。该 F2 anchor 不能进入 T1；同一科学输入不再重试，父 15 行分母保持零信用。后审报告见 [F2-STATIC-RECEIVER-BALLISTIC-CATCH-SOLVER-ANCHOR-2026-09-21.zh-CN.md](F2-STATIC-RECEIVER-BALLISTIC-CATCH-SOLVER-ANCHOR-2026-09-21.zh-CN.md)。Core gate 继续为 T1 `F3/F4`、宏观 T2 `0/2`、formal training `0/9`。
