...
`qualification_row_sha256` 的算法固定为：对 scope receipt `matrix.rows[i]` 的完整 row object 先 strict-parse，再用 V12 `canonical_json_bytes` 编码（UTF-8；`json.dumps(sort_keys=True,separators=(",",":"),allow_nan=False)` 默认 ASCII escaping；不附加 LF），对所得 bytes 计算 SHA-256。该 digest 绑定行内容；scope receipt 原始 bytes 另由 `qualification_matrix_raw_sha256` 绑定。任何未按此规则计算的 row digest 都拒绝；这只规定可复算的身份编码，不认证 scope receipt 来源。
