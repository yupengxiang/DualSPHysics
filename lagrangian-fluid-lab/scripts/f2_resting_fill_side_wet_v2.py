...
SCHEMA = "core.f2.resting_fill_side_wet.v2"
JOB_SCHEMA = "core.cfd.job.v1"
REVISION_ID = "F2_resting_fill_side_wet_full_cup_v2"
SCOPE_ID = "F2_resting_fill_side_wet_full_cup_x_v2"
CASE_ID = "CORE_F2_resting_fill_side_wet_full_cup_q0p50000000_dp0p007500000000_canary"
SOURCE_RELATIVE = "campaigns/l2-multifamily/c1-canary/cases/L2_C1_F2_rotation_center_nominal_Def.xml"
MOTION_NAME = "CORE_F2_resting_fill_side_wet_full_cup_zero_motion.dat"
DP_M = 0.0075
TIME_MAX_S = 0.60
DRIVE_START_S = 0.50
OUTPUT_INTERVAL_S = 0.02
TARGET_FIRST_CENTER = [0.005, -0.150, 0.660]
SOURCE_DRAW_POINT = [0.00625, -0.15125, 0.66125]
SOURCE_DRAW_SIZE = [0.420, 0.300, 0.1725]
NATIVE_COUNTS = [57, 41, 24]
CONTINUUM_BOX = {
    "low": [0.0, -0.15, 0.65],
    ...
    with tempfile.TemporaryDirectory(prefix="f2-side-wet-preflight-") as temp:
        ...
        checks = {
            ...
            "target_native_position_bounds": bool(
                ...
                and np.allclose(fluid_positions.max(axis=0), [0.425, 0.150, 0.8325], atol=1e-10)
            ),
        ...
        return {
            "schema": "core.f2.resting_fill_side_wet.static_preflight.v2",
...
def _config(preflight_path: Path) -> dict:
    return {
        ...
        "revision_id": REVISION_ID,
        "case_id": CASE_ID,
        "recipe_id": "F2_resting_fill_side_wet_full_cup_dbc_native_v2",
        "recipe": "native_dbc",
        "stage": "repair_canary",
        ...
        "physical_case_id": "F2_resting_fill_side_wet_full_cup_fixed_volume_geometry_v2",
...
def run(prepared_path: Path, lab: Path, output: Path) -> dict:
    ...
    audit = {
        "schema": "core.f2.resting_fill_side_wet.audit.v2",
        "revision_id": REVISION_ID,
...
def make_job(prepared_path: Path, lab: Path, output: Path) -> dict:
    ...
    script = (lab / "scripts/f2_resting_fill_side_wet_v2.py").resolve()
    observer_script = (lab / "scripts/core_f2_resting_fill.py").resolve()
    spec = {
        "schema": JOB_SCHEMA,
        "job_id": "f2-resting-fill-side-wet-full-cup-canary-v2-001",
        "logical_id": "f2-resting-fill-side-wet-full-cup-canary-v2-001",
        "attempt_role": "independent_full_cup_side_wetted_initial_condition_static_hold",
        "category": "f2_resting_fill_side_wet_full_cup_canary",
        "host": "h200",
        "source_lab": str(lab),
        "cwd": str(lab),
