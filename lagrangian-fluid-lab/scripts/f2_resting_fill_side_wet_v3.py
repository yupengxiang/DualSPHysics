...
SCHEMA = "core.f2.resting_fill_side_wet.v3"
JOB_SCHEMA = "core.cfd.job.v1"
REVISION_ID = "F2_resting_fill_side_wet_full_cup_v3"
SCOPE_ID = "F2_resting_fill_side_wet_full_cup_x_v3"
CASE_ID = "CORE_F2_resting_fill_side_wet_full_cup_q0p50000000_dp0p007500000000_canary_v3"
SOURCE_RELATIVE = "campaigns/l2-multifamily/c1-canary/cases/L2_C1_F2_rotation_center_nominal_Def.xml"
MOTION_NAME = "CORE_F2_resting_fill_side_wet_full_cup_v3_zero_motion.dat"
DP_M = 0.0075
TIME_MAX_S = 0.60
DRIVE_START_S = 0.50
OUTPUT_INTERVAL_S = 0.02
TARGET_FIRST_CENTER = [0.0025, -0.145, 0.660]
SOURCE_DRAW_POINT = [0.00125, -0.14875, 0.66125]
SOURCE_DRAW_SIZE = [0.420, 0.2925, 0.1725]
NATIVE_COUNTS = [57, 40, 24]
CONTINUUM_BOX = {
    "low": [0.0, -0.15, 0.65],
    "size": [0.425, 0.30, 0.18505882352941177],
    "mkfluid": 0,
}
    ...
    with tempfile.TemporaryDirectory(prefix="f2-side-wet-preflight-") as temp:
        ...
        checks = {
            ...
            "target_native_position_bounds": bool(
                ...
                and np.allclose(fluid_positions.max(axis=0), [0.4225, 0.1475, 0.8325], atol=1e-10)
            ),
        ...
        return {
            "schema": "core.f2.resting_fill_side_wet.static_preflight.v3",
            ...
            "no_overlap_definition": "continuous fluid faces are the physical cup interior faces; native fluid centers use a separately declared inset lattice, all decoded centers are inside the physical cup bounds, have positive distance from generated cup particles, and meet native support; no mass or geometry rescaling",
        }
...
def _config(preflight_path: Path) -> dict:
    return {
        ...
        "revision_id": REVISION_ID,
        "case_id": CASE_ID,
        "recipe_id": "F2_resting_fill_side_wet_full_cup_dbc_native_v3",
        "recipe": "native_dbc",
        "stage": "repair_canary",
        "split": "qualification_only",
        "qualification_only": True,
        "parameter": {"name": "initial_condition_variant", "value": "side_wet_full_cup_fixed_volume", "range": ["side_wet_full_cup_fixed_volume"]},
        "dp_m": DP_M,
        "cfl": 0.2,
        "time_max_s": TIME_MAX_S,
        "output_interval_s": OUTPUT_INTERVAL_S,
        "source_definition": SOURCE_RELATIVE,
        "physical_case_id": "F2_resting_fill_side_wet_full_cup_fixed_volume_geometry_v3",
        ...
        "initial_condition": {
            "variant": "side_wet_full_cup_fixed_volume",
            "continuum_wall_geometry_changed": False,
            "fluid_initial_condition_changed": True,
            "continuous_fluid_faces_are_physical_cup_faces": True,
            "side_wetted_candidate": True,
            "continuum_box_matches_cup_interior_faces": True,
            "continuous_volume_m3": float(np.prod(CONTINUUM_BOX["size"])),
            "predecessor_continuous_volume_m3": 0.023595,
            "continuum_height_derivation": "0.023595/(0.425*0.30)=0.18505882352941177 m",
            ...
            "native_sampling_is_separate_from_continuum_faces": True,
            "native_sampling_source_point_m": SOURCE_DRAW_POINT,
            "native_sampling_source_size_m": SOURCE_DRAW_SIZE,
        },
        "static_hold": {
        ...
        "qualification_inheritance": "none; independent full-cup side-wetted initial-condition scope",
        "qualification_claim": "none; independent full-cup side-wetted static-hold canary only",
        "qualified": False,
    }
...
def prepare_canary(lab: Path, output: Path) -> dict:
    ...
    config["sampling_rule"] = "continuous fluid box exactly matches the cup interior faces; native sampling uses a separately declared inset lattice, native rho*dp^3, no rescaling"
    ...
    prepared = {
        ...
        "solver_arguments": [],
        "qualification_only": True,
        "qualification_claim": "none; independent full-cup side-wetted resting-fill static-hold canary only",
    }
...
def run(prepared_path: Path, lab: Path, output: Path) -> dict:
    ...
    audit = {
        "schema": "core.f2.resting_fill_side_wet.audit.v3",
        "revision_id": REVISION_ID,
        ...
        "qualified": False,
        "qualification_claim": "none; independent full-cup side-wetted static-hold canary audit",
    }
    ...
    result["qualification_claim"] = "none; independent full-cup side-wetted static-hold canary only"
...
def make_job(prepared_path: Path, lab: Path, output: Path) -> dict:
    ...
    script = (lab / "scripts/f2_resting_fill_side_wet_v3.py").resolve()
    observer_script = (lab / "scripts/core_f2_resting_fill.py").resolve()
    spec = {
        "schema": JOB_SCHEMA,
        "job_id": "f2-resting-fill-side-wet-full-cup-canary-v3-001",
        "logical_id": "f2-resting-fill-side-wet-full-cup-canary-v3-001",
        "attempt_role": "independent_full_cup_side_wetted_initial_condition_static_hold",
        "category": "f2_resting_fill_side_wet_full_cup_v3_canary",
        "host": "h200",
        "source_lab": str(lab),
        "cwd": str(lab),
        ...
        "qualification_claim": "none",
        "qualification_only": True,
        "qualification_status": "candidate-only; independent full-cup side-wetted initial condition; no moving-cup/range qualification",
        "input_files": [
