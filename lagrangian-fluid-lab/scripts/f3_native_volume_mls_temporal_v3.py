"""F3 native-volume MLS with explicitly bounded temporal field interpolation.

This is an independent CPU-only v3 backend.  It reconstructs a reference
velocity field from native mass/density/position/velocity arrays at the two
registered saved frames bracketing the *current* integration interval.  Each
RK stage linearly interpolates those endpoint arrays at its stage time.  Native
particle positions are interpolated only as numerical support geometry; the
independent tracer path is obtained by RK4 and is never read from native
particle labels or trajectories.  No frame outside that interval and no
model CFD state is readable through the provider.

The trace is diagnostic evidence only.  It binds the registered F3 geometry
and the original F3 support gate as comparison fields, while the native-volume
MLS numerical support checks and temporal interpolation are independently
versioned.  No ledger, scheduler slot, GPU, or CFD solver is touched.
"""
...
SCHEMA = "core.material.f3.native_volume_mls.temporal.v3"
BACKEND = "f3_native_volume_mls_wendland_mass_density_temporal_v3"
...
CHECKPOINT_SCHEMA = "core.material.f3.native_volume_mls.checkpoint.temporal.v3"
CHECKPOINT_FIELDS = (
...
def _selection_hash(indices: np.ndarray) -> str:
    """Hash the logical-to-native frame map with its dtype and shape."""
    indices = np.ascontiguousarray(np.asarray(indices, dtype=np.int64))
    return array_hash(indices)


def _load_frame_index_map(path: str | Path, source: Path,
                          source_times: np.ndarray) -> tuple[np.ndarray, dict]:
    """Load and validate a direct native-frame selection.

    Cadence comparisons select saved native frames.  This function never
    creates a time, position, or velocity by interpolation.  The selection
    includes both source endpoints so a complete-window trace preserves the
    dense source horizon.
    """
    path = Path(path).resolve()
    value = json.loads(path.read_text(encoding="utf-8"))
    if value.get("schema") != "core.material.f3.native_volume_mls.frame_selection.v1":
        raise ValueError("unsupported native frame-selection schema")
    expected_source = str(source.resolve())
    if value.get("source_h5") not in (expected_source, str(source)):
        raise ValueError("frame-selection source does not match --source")
    source_sha = sha256_file(source)
    if value.get("source_sha256") != source_sha:
        raise ValueError("frame-selection source hash does not match --source")
    raw = value.get("source_frame_indices")
    if not isinstance(raw, list) or not raw:
        raise ValueError("frame-selection source_frame_indices must be a nonempty list")
    if any(isinstance(item, (bool, np.bool_))
           or not isinstance(item, (int, np.integer)) for item in raw):
        raise ValueError("frame-selection indices must be non-bool integers")
    try:
        indices = np.asarray(raw, dtype=np.int64)
    except (TypeError, ValueError) as exc:
        raise ValueError("frame-selection indices must be integers") from exc
    if indices.ndim != 1 or np.any(indices < 0) or np.any(indices >= len(source_times)):
        raise ValueError("frame-selection indices are outside the source time axis")
    if indices[0] != 0 or indices[-1] != len(source_times) - 1:
        raise ValueError("frame-selection must include source first and last frames")
    if len(indices) < 2 or np.any(np.diff(indices) <= 0):
        raise ValueError("frame-selection indices must be strictly increasing")
    expected_hash = _selection_hash(indices)
    if value.get("selection_sha256") != expected_hash:
        raise ValueError("frame-selection index hash mismatch")
    selected_times = np.asarray(source_times[indices], dtype=np.float64)
    if not np.isfinite(selected_times).all() or np.any(np.diff(selected_times) <= 0):
        raise ValueError("frame-selection selected time axis is not strictly increasing")
    nominal = value.get("target_nominal_interval_s")
    if nominal is None or not np.isfinite(float(nominal)) or float(nominal) <= 0.0:
        raise ValueError("frame-selection target_nominal_interval_s must be positive")
    # The target is a cadence diagnostic.  Timestamps remain the actual
    # selected native times, with solver jitter bounded by the manifest.
    target = np.arange(len(indices), dtype=np.float64) * float(nominal)
    tolerance = float(value.get("target_time_tolerance_s", max(5.0e-5, 0.25 * float(nominal))))
    if not np.isfinite(tolerance) or tolerance < 0.0:
        raise ValueError("frame-selection target_time_tolerance_s must be finite and nonnegative")
    if np.max(np.abs(selected_times - target)) > tolerance:
        raise ValueError("frame-selection selected times exceed declared cadence tolerance")
    metadata = {
        "schema": value["schema"],
        "path": str(path),
        "sha256": sha256_file(path),
        "source_h5": expected_source,
        "source_sha256": source_sha,
        "source_frame_count": int(len(source_times)),
        "logical_frame_count": int(len(indices)),
        "selection_sha256": expected_hash,
        "selection_mode": value.get("selection_mode", "direct_native_frame_selection"),
        "interpolation": False,
        "target_nominal_interval_s": float(nominal),
        "target_time_tolerance_s": tolerance,
        "selected_time_start_s": float(selected_times[0]),
        "selected_time_end_s": float(selected_times[-1]),
        "selected_interval_min_s": float(np.min(np.diff(selected_times))),
        "selected_interval_median_s": float(np.median(np.diff(selected_times))),
        "selected_interval_max_s": float(np.max(np.diff(selected_times))),
...
class F3CurrentFrame:
    """Immutable native arrays for one registered saved frame.

    ``native_indices`` keeps row correspondence explicit for temporal
    interpolation.  It is a native particle-axis locator only; it is never a
    tracer identity or source label.
    """

    __slots__ = ("position", "velocity", "mass", "density", "valid", "native_indices",
                 "frame_index", "time_s")

    def __init__(self, position, velocity, mass, density, valid, *, frame_index: int,
                 time_s: float, native_indices=None):
        ...
        if native_indices is None:
            native_indices = np.arange(len(position), dtype=np.int64)
        native_indices = np.asarray(native_indices, dtype=np.int64)
        if native_indices.shape != (len(position),) or np.any(native_indices < 0):
            raise ValueError("native_indices must have shape [N] and be nonnegative")
        ...
        self.native_indices = np.array(native_indices, dtype=np.int64, copy=True)
...
class F3TemporalReferenceProvider:
    """Two-frame cache with interval-bounded linear reference-field access.

    ``field_at(i, t)`` is legal only for ``times[i] <= t <= times[i+1]`` and
    reads exactly those two registered native frames.  The provider has no
    API for a frame beyond the interval, and it never opens a model CFD
    source.  The interpolated arrays are an explicit numerical field model;
    they do not claim that native particle trajectories were interpolated.
    """

    provider_role = "reference_native_saved_frame_pair_linear_temporal_v3"

    def __init__(self, source: str | Path, *, fluid_type: int = F3_FLUID_TYPE,
                 max_cache: int = 2, frame_index_map: str | Path | None = None):
        ...
        self.source_times = np.asarray(self.h5["time"][:], dtype=np.float64)
        if (self.source_times.ndim != 1 or len(self.source_times) < 2
                or not np.isfinite(self.source_times).all()
                or np.any(np.diff(self.source_times) <= 0.0)):
            self.close()
            raise ValueError("F3 source time must be strictly increasing")
        n = self.h5["position"].shape[1]
        expected_shapes = {
            "position": (len(self.source_times), n, 3),
            "velocity": (len(self.source_times), n, 3),
            "mass": (len(self.source_times), n),
            "density": (len(self.source_times), n),
            "valid": (len(self.source_times), n),
            "type": (len(self.source_times), n),
        }
        ...
        if frame_index_map is None:
            self.frame_indices = np.arange(len(self.source_times), dtype=np.int64)
            self.frame_selection = {
                "schema": "core.material.f3.native_volume_mls.frame_selection.identity.v1",
                "path": None,
                "sha256": None,
                "source_h5": str(self.source),
                "source_sha256": self.source_sha256,
                "source_frame_count": int(len(self.source_times)),
                "logical_frame_count": int(len(self.source_times)),
                "selection_sha256": _selection_hash(self.frame_indices),
                "selection_mode": "identity_all_native_frames",
                "interpolation": False,
                "target_nominal_interval_s": float(np.median(np.diff(self.source_times))),
                "target_time_tolerance_s": None,
                "selected_time_start_s": float(self.source_times[0]),
                "selected_time_end_s": float(self.source_times[-1]),
                "selected_interval_min_s": float(np.min(np.diff(self.source_times))),
                "selected_interval_median_s": float(np.median(np.diff(self.source_times))),
                "selected_interval_max_s": float(np.max(np.diff(self.source_times))),
                "source_frame_indices": self.frame_indices.tolist(),
            }
        else:
            self.frame_indices, self.frame_selection = _load_frame_index_map(
                frame_index_map, self.source, self.source_times)
        self.frame_indices.setflags(write=False)
        self.times = np.asarray(self.source_times[self.frame_indices], dtype=np.float64)
        self.times.setflags(write=False)
        self.frame_count = int(len(self.frame_indices))
    ...
    def frame(self, index: int) -> F3CurrentFrame:
        ...
        source_index = int(self.frame_indices[index])
        h = self.h5
        position = np.asarray(h["position"][source_index], dtype=np.float64)
        velocity = np.asarray(h["velocity"][source_index], dtype=np.float64)
        mass = np.asarray(h["mass"][source_index], dtype=np.float64)
        density = np.asarray(h["density"][source_index], dtype=np.float64)
        valid = np.asarray(h["valid"][source_index], dtype=bool)
        fluid = np.asarray(h["type"][source_index], dtype=np.float64) == self.fluid_type
        valid &= fluid
        ...
        frame = F3CurrentFrame(
            ...
            frame_index=source_index, time_s=float(self.times[index]),
            native_indices=np.flatnonzero(valid),
        )
        self.cache[index] = frame
        self.cache.move_to_end(index)
        self.loaded_indices.append(source_index)
        while len(self.cache) > self.max_cache:
            self.cache.popitem(last=False)
        return frame

    def endpoint_pair(self, interval_index: int) -> tuple[F3CurrentFrame, F3CurrentFrame]:
        """Return only the two saved endpoint frames for one interval."""
        interval_index = int(interval_index)
        if not 0 <= interval_index < self.frame_count - 1:
            raise IndexError(
                f"temporal interval {interval_index} is outside 0..{self.frame_count - 2}"
            )
        left = self.frame(interval_index)
        right = self.frame(interval_index + 1)
        if not np.array_equal(left.native_indices, right.native_indices):
            raise ValueError(
                "temporal interpolation requires identical valid fluid native rows "
                "at both registered interval endpoints"
            )
        return left, right

    def field_at(self, interval_index: int, time_s: float) -> F3CurrentFrame:
        """Linearly interpolate a field within one registered interval.

        The endpoint arrays are native saved-frame data.  Their position,
        velocity, mass, and density arrays are interpolated as support-field
        data; ``time_s`` is clamped only by roundoff at the interval
        endpoints.  Any true request outside the interval is rejected, which
        prevents accidental use of a future CFD frame or extrapolation.  The
        independent tracer path remains the RK4 state.
        """
        left, right = self.endpoint_pair(interval_index)
        t0 = float(self.times[int(interval_index)])
        t1 = float(self.times[int(interval_index) + 1])
        time_s = float(time_s)
        if not np.isfinite(time_s):
            raise ValueError("temporal field time must be finite")
        tolerance = max(1.0e-13, 32.0 * np.finfo(np.float64).eps * max(1.0, abs(t0), abs(t1)))
        if time_s < t0 - tolerance or time_s > t1 + tolerance:
            raise ValueError(
                f"temporal field request {time_s} is outside registered interval [{t0}, {t1}]"
            )
        query_time = min(max(time_s, t0), t1)
        ...
        return F3CurrentFrame(
            one_minus * left.position + alpha * right.position,
            one_minus * left.velocity + alpha * right.velocity,
            one_minus * left.mass + alpha * right.mass,
            one_minus * left.density + alpha * right.density,
            np.ones(len(left.position), dtype=bool),
            frame_index=left.frame_index,
            time_s=query_time,
            native_indices=left.native_indices,
        )
...
# Compatibility within this independent v3 module only.  The explicit class
# name is used by the binding and keeps the old v1/v2 providers untouched.
F3ReferenceProvider = F3TemporalReferenceProvider
...
class F3NativeVolumeMLS:
    ...
    def binding(self) -> dict:
        return {
            ...
            "future_state": "forbidden_beyond_current_interval_endpoint",
            "temporal_field": "linear interpolation of the two registered saved endpoint arrays, including support positions",
            "temporal_scope": "current_interval_only_no_extrapolation",
            "model_source": "forbidden; reference source H5 only",
            "native_particle_id": "not used for query identity or termination",
            "tracer_path": "independent RK4 state; no native particle trajectory is used as a tracer path",
        }
...
def _trace_binding(source: Path, provider: F3TemporalReferenceProvider, tracer: F3NativeVolumeMLS,
    ...
    return {
        ...
        "frame_policy": (
            "linear interpolation of native position/velocity/mass/density arrays "
            "between exactly the two registered selected frames bracketing each "
            "current integration interval; no extrapolation"
        ),
        "frame_selection": dict(provider.frame_selection),
        "source_frame_count": int(len(provider.source_times)),
        "logical_frame_count": int(provider.frame_count),
        "interpolation": True,
        "interpolation_kind": "componentwise_linear_saved_field_arrays",
        "interpolation_scope": "current_interval_endpoints_only",
        "future_velocity": "forbidden_beyond_current_interval_endpoint",
        "future_density": "forbidden_beyond_current_interval_endpoint",
        "model_source": "forbidden; provider opens only registered reference source_h5",
        "support_native_particle_positions": "linearly interpolated between current interval endpoints",
        "tracer_path": "independent RK4 state; not interpolated from native particle labels",
        "substeps": int(substeps),
        "stop_after": int(stop_after),
        "query_count": int(len(initial)),
        "seed_hash": array_hash(
            initial,
            labels,
            np.asarray([f"f3-seed-{i:06d}" for i in range(len(initial))], dtype=object).astype(str),
        ),
        "source_definition": {
...
def _advance_rk4(state: dict, tracer: F3NativeVolumeMLS, provider: F3TemporalReferenceProvider,
                 walls: np.ndarray, interval_index: int, segment_time: float, dt: float,
                 diagnostics: dict) -> MLSResult:
    """Advance one RK4 segment using the current interval's field only.

    Every stage obtains a field at its stage time from the same two registered
    endpoint frames.  In particular, k4 may use the right endpoint but can
    never ask the provider for the following interval or extrapolate beyond
    this one.
    """
    q = state["position"]
...
def _summary(handle, binding: dict, labels: np.ndarray, provider: F3TemporalReferenceProvider,
    ...
    return {
        "schema": "core.material.f3.native_volume_mls.full_cadence_result.temporal.v3",
        ...
        "source_window": {
            ...
            "native_frame_policy": (
                "selected native source frames are the only field endpoints; each "
                "accepted interval uses componentwise linear interpolation between "
                "its two endpoints"
            ),
            "temporal_field_policy": "current_interval_endpoints_only_no_extrapolation",
            "frame_selection": binding.get("frame_selection"),
        },
...
def audit_source(source: str | Path, prepared: str | Path, *, dp_m: float | None = None,
                 frame_index_map: str | Path | None = None) -> dict:
    ...
    with h5py.File(source, "r") as handle:
        ...
        if (times.ndim != 1 or len(times) != frame_count or len(times) < 2
                or not np.isfinite(times).all() or np.any(np.diff(times) <= 0.0)):
            raise ValueError("F3 source time must be finite and strictly increasing")
        if frame_index_map is None:
            selected_indices = np.arange(frame_count, dtype=np.int64)
            frame_selection = {
                "schema": "core.material.f3.native_volume_mls.frame_selection.identity.v1",
                "path": None,
                "sha256": None,
                ...
                "logical_frame_count": int(frame_count),
                "selection_sha256": _selection_hash(selected_indices),
                "selection_mode": "identity_all_native_frames",
                "interpolation": False,
                "target_nominal_interval_s": float(np.median(np.diff(times))),
                "target_time_tolerance_s": None,
                "selected_time_start_s": float(times[0]),
                "selected_time_end_s": float(times[-1]),
                "selected_interval_min_s": float(np.min(np.diff(times))),
                "selected_interval_median_s": float(np.median(np.diff(times))),
                "selected_interval_max_s": float(np.max(np.diff(times))),
                "source_frame_indices": selected_indices.tolist(),
            }
        else:
            selected_indices, frame_selection = _load_frame_index_map(
                frame_index_map, source, times)
        selected_times = times[selected_indices]

        # Preflight reads only the initial field.  Endpoint row correspondence
        # is checked lazily by endpoint_pair() when that interval becomes the
        # current integration interval; this keeps future field rows outside
        # the runner's read permission.
        ...
        velocity0 = np.asarray(handle["velocity"][0], dtype=np.float64)
        mass0 = np.asarray(handle["mass"][0], dtype=np.float64)
        density0 = np.asarray(handle["density"][0], dtype=np.float64)
        valid_rows = (
            valid0 & (type0 == F3_FLUID_TYPE)
            & np.isfinite(position0).all(axis=1)
            & np.isfinite(velocity0).all(axis=1)
            & np.isfinite(mass0) & (mass0 > 0.0)
            & np.isfinite(density0) & (density0 > 0.0)
        )
        initial_mass_sum = float(mass0[valid_rows].sum())
    seeds = seeds_f3(512)
    labels = source_labels(seeds)
    walls = f3_walls()
    return {
        "schema": "core.material.f3.native_volume_mls.source_preflight.temporal.v3",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        ...
        "source": {
            ...
            "time_start_s": float(times[0]),
            "time_end_s": float(selected_times[-1]),
            "native_output_interval_nominal_s": F3_OUTPUT_INTERVAL_S,
            "time_step_min_s": float(np.min(np.diff(selected_times))),
            "time_step_median_s": float(np.median(np.diff(selected_times))),
            "time_step_max_s": float(np.max(np.diff(selected_times))),
            "selected_source_frame_indices_sha256": _selection_hash(selected_indices),
            "interpolation": True,
            "interpolation_kind": "componentwise_linear_saved_field_arrays",
            "interpolation_scope": "current_interval_endpoints_only",
            "datasets": required,
            ...
            "current_frame_mass_density": "endpoint_mass_density_linear_field",
            "initial_native_mass_sum_kg": initial_mass_sum,
            "endpoint_row_mask_check": "performed lazily for the current interval pair; no future endpoint field is pre-read",
        ...
        "candidate_binding": {
            ...
            "backend": BACKEND,
            "future_state": "forbidden_beyond_current_interval_endpoint",
            "native_volume": "linearly interpolated mass/density from current interval endpoints",
            "support": "all current valid fluid rows within 2h after finite-wall visibility",
            "fit": "weighted affine MLS SVD",
            "temporal_field": "linear saved-field arrays; native particle trajectories not interpolated",
            "model_source": "forbidden; registered reference source H5 only",
            "valid_fluid_native_rows": "identical row mask required at every selected endpoint",
...
def run_trace(source: str | Path, output: str | Path, prepared: str | Path, *,
              ...
              frame_index_map: str | Path | None = None,
    ...
    audit = audit_source(source, prepared, dp_m=dp_m, frame_index_map=frame_index_map)
    ...
    with F3TemporalReferenceProvider(source, frame_index_map=frame_index_map) as provider:
        ...
        initial_frame = provider.field_at(0, float(provider.times[0]))
        ...
        with h5py.File(output, "r+") as handle:
            for next_index in range(start_frame, end + 1):
                ...
                left_endpoint, right_endpoint = provider.endpoint_pair(frame_index)
                ...
                for substep_index in range(int(substeps)):
                    ...
                    result = _advance_rk4(
                        state, tracer, provider, walls, frame_index, segment_time, dt, diagnostics
                    )
                if result is None:
                    raise RuntimeError("no RK stage result")
                values = _frame_values(state, initial_result, result,
                                       float(right_endpoint.mass.sum()), weights)
...
def _cli() -> int:
    ...
    parser.add_argument("--frame-index-map", type=Path,
                        help="direct native source-frame selection JSON; no interpolation")
    ...
    audit = audit_source(args.source, args.prepared, dp_m=args.dp,
                         frame_index_map=args.frame_index_map)
