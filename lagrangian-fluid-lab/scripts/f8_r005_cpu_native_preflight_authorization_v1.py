#!/usr/bin/env python3
"""Seal the single F8 r005 CPU/native preflight admission; never run binaries."""
...
from scripts import f8_r005_input_materialization_v1 as inputs
...
RUNNER = Path("scripts/f8_r005_cpu_native_preflight_runner_v1.py")
EXECUTOR = Path("scripts/f8_r005_cpu_native_preflight_execute_v1.py")
...
def validate_predecessors() -> dict[str, Any]:
    ...
    if not (
        ...
        raise PermissionError("F8 r005 static review is not current, passed, and zero-credit")
    if not (
        ...
        raise PermissionError("F8 r005 materialization is not current, immutable, and hash-closed")
    ...
    if not (
        ...
        raise ValueError("F8 r005 materialized inputs and control-copy contract do not match the static review")

    values = design.parameters()
    dp, height = float(values["dp"]), float(values["half_height"])
    nx = round(float(values["length_x"]) / dp)
    ny = round(float(values["length_y"]) / dp)
    fluid_layers = round((2.0 * height) / dp) + 1
    expected_fluid = int(nx * ny * fluid_layers)
    expected_boundary = int(2 * nx * ny)
    return {
        "definition": definition,
        "control": control,
        "control_copy": copy,
        "constantsdef": materialization["definition_proof"]["constantsdef"],
        "expected_wall_plane_particle_counts": {"-0.0525": int(nx * ny), "0.0525": int(nx * ny)},
...
def build_authorization() -> dict[str, Any]:
    ...
    return {
        "schema": "core.cfd.f8.r005_cpu_native_preflight_authorization.v1",
        "scope_id": SCOPE,
        "status": "authorized_for_exactly_one_r005_cpu_native_preflight",
        "authorization_basis": "user-wide exploration authorization plus the hash-closed r005 static review and input materialization",
        "qualification_claim": "none",
        "qualification_credit": 0,
        "closed_prior_scopes": load(REVIEW)["closed_prior_execution_scopes"],
        "single_input": {
            "definition": prior["definition"], "control": prior["control"],
            "r001_r002_r003_r004_inputs_or_outputs_reused": False,
        ...
        "hard_gates": {
            "input_constantsdef_matches_r005_contract": True,
        ...
        "execution_policy": {
            "exactly_one_cpu_gencase": True, "exactly_one_native_decode": True,
        ...
        "bindings": [
            binding(REVIEW, "passed immutable F8 r005 static design review"),
            binding(MATERIALIZATION, "immutable F8 r005 input materialization receipt"),
            binding(inputs.DEFINITION_TARGET, "only permitted r005 Definition"),
            binding(inputs.CONTROL_TARGET, "only permitted r005 colocated acceleration control"),
            binding(GENCASE, "official CPU GenCase binary"),
            binding(DECODER, "pinned native BI4 decoder"),
            binding(Path("scripts/f8_r005_cpu_native_preflight_authorization_v1.py"), "r005 one-shot authorization builder"),
            binding(RUNNER, "r005 argv-only admission runner"),
            binding(EXECUTOR, "r005 exactly-once preflight executor"),
            binding(NATIVE_HELPER, "pinned native BI4 array decoder and CPU environment"),
            binding(Path("tests/test_f8_r005_cpu_native_preflight_v1.py"), "r005 preflight admission and gate tests"),
        ],
    }
...
def write_authorization(path: Path = OUTPUT) -> dict[str, Any]:
    ...
    if target.exists():
        raise FileExistsError(f"refusing to overwrite immutable F8 r005 authorization: {target}")
