...
This wrapper versions the immutable receipt and one-shot namespace while
reusing the reviewed, read-only v2 probes. It cannot launch or reserve a worker.
"""
...
SCOPE_DIR = LAB / "campaigns/core-v1/material/evidence/f3-material-row30-resource-preflight-v3"
OUTPUT = SCOPE_DIR / "receipt.json"
LOCK = SCOPE_DIR / "one-shot-lock.json"
SCHEMA = "core.material.f3.row30.resource_preflight.v3"
RECORD_ID = "f3-material-row30-resource-preflight-v3"
...
def build(
    *,
    ...
    value["supersedes"] = preflight_v2._binding(
        preflight_v2.OUTPUT.relative_to(LAB),
        "prior blocked row30 v2 preflight; preserved as historical evidence",
        preflight_v2._read_regular(preflight_v2.OUTPUT),
    )
    value["bindings"].extend((
        preflight_v2._binding(
            Path("scripts/f3_material_row30_resource_preflight_v3.py"),
            "v3 one-shot preflight wrapper",
            preflight_v2._read_regular(LAB / "scripts/f3_material_row30_resource_preflight_v3.py"),
        ),
        preflight_v2._binding(
            Path("tests/test_f3_material_row30_resource_preflight_v3.py"),
            "v3 preflight contract tests",
            preflight_v2._read_regular(LAB / "tests/test_f3_material_row30_resource_preflight_v3.py"),
        ),
    ))
    return value
...
def write_once(*, scope_dir: Path = SCOPE_DIR) -> dict[str, Any]:
    ...
    if scope.exists() or scope.is_symlink():
        raise FileExistsError(f"F3 row-30 v3 one-shot namespace already exists: {scope}")
    scope.mkdir(mode=0o755)
    lock = scope / LOCK.name
    lock_value = {
        "schema": "core.material.f3.row30.resource_preflight_one_shot_lock.v1",
