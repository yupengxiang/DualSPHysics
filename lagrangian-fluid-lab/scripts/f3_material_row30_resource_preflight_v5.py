#!/usr/bin/env python3
"""Consume one explicitly authorized fresh F3 row-30 resource preflight.

This additive v5 wrapper binds the prior v4 one-shot receipt and current
read-only scheduler/resource probes. It never starts a worker or reserves a
queue/ledger slot; a blocked result is an immutable terminal observation.
"""
...
SCOPE_DIR = LAB / "campaigns/core-v1/material/evidence/f3-material-row30-resource-preflight-v5"
OUTPUT = SCOPE_DIR / "receipt.json"
LOCK = SCOPE_DIR / "one-shot-lock.json"
SCHEMA = "core.material.f3.row30.resource_preflight.v5"
RECORD_ID = "f3-material-row30-resource-preflight-v5"
AUTHORITY = "explicit user authorization: one new F3 material row-30 resource/scheduler preflight; no worker launch"
PRIOR_RECEIPT = Path("campaigns/core-v1/material/evidence/f3-material-row30-resource-preflight-v4/receipt.json")
...
def build(*, environment_probe=None, process_probe=None, now: datetime | None = None) -> dict[str, Any]:
    ...
    value["supersedes"] = preflight_v2._binding(
        PRIOR_RECEIPT,
        "prior blocked row30 v4 preflight; preserved as historical evidence",
        preflight_v2._read_regular(LAB / PRIOR_RECEIPT),
    )
    value["bindings"].extend((
        preflight_v2._binding(
            Path("scripts/f3_material_row30_resource_preflight_v5.py"),
            "v5 fresh one-shot preflight wrapper",
            preflight_v2._read_regular(LAB / "scripts/f3_material_row30_resource_preflight_v5.py"),
        ),
        preflight_v2._binding(
            Path("tests/test_f3_material_row30_resource_preflight_v5.py"),
            "v5 preflight contract tests",
            preflight_v2._read_regular(LAB / "tests/test_f3_material_row30_resource_preflight_v5.py"),
        ),
    ))
    return value
...
def write_once(*, scope_dir: Path = SCOPE_DIR) -> dict[str, Any]:
    ...
    if scope.exists() or scope.is_symlink():
        raise FileExistsError(f"F3 row-30 v5 one-shot namespace already exists: {scope}")
