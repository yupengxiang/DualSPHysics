#!/usr/bin/env python3
"""Seal one CPU-only F8 r007 GenCase/native-decoder preflight; never run binaries."""
...
from scripts import f8_r007_input_materialization_v1 as inputs
...
RUNNER = Path("scripts/f8_r007_cpu_native_preflight_runner_v1.py")
EXECUTOR = Path("scripts/f8_r007_cpu_native_preflight_execute_v1.py")
SHARED_AUDITOR = Path("scripts/f8_r006_cpu_native_preflight_execute_v1.py")
SHARED_AUDITOR_TESTS = Path("tests/test_f8_r006_cpu_native_preflight_v1.py")
...
R006_PREFLIGHT = design.R006_PREFLIGHT
R006_AUTHORIZATION = design.R006_AUTHORIZATION
...
def validate_predecessors() -> dict[str, Any]:
    ...
    previous_failure, previous_authorization = load(R006_PREFLIGHT), load(R006_AUTHORIZATION)
    ...
    if not (
        ...
        and review.get("status") == "r007_static_design_review_passed_inputs_not_authorized"
        and review.get("static_constraint_gaps") == []
        and review.get("qualification_credit") == 0
        and bindings_current(review)
    ):
        raise PermissionError("F8 r007 static review is not current, passed, and zero-credit")
    if not (
        materialization.get("schema") == inputs.SCHEMA
        and materialization.get("scope_id") == SCOPE
        and materialization.get("status") == "one_time_r007_inputs_materialized_static_only"
        ...
        raise PermissionError("F8 r007 materialization is not current, immutable, and hash-closed")
    if not (
        previous_authorization.get("status") == "authorized_for_exactly_one_r006_cpu_native_preflight"
        and previous_authorization.get("execution_policy", {}).get("same_input_retry_forbidden") is True
        and previous_authorization.get("execution_policy", {}).get("preflight_output_reuse_forbidden") is True
        and previous_failure.get("schema") == "core.cfd.f8.r006_cpu_native_preflight.v1"
        and previous_failure.get("status") == "generated_geometry_failed_hard_audit"
        and previous_failure.get("qualification_credit") == 0
        and previous_failure.get("execution_controls", {}).get("native_decode_invoked") is False
        and previous_failure.get("execution_controls", {}).get("solver_invoked") is False
        and previous_failure.get("execution_controls", {}).get("gpu_invoked") is False
    ):
        raise PermissionError("r006 is not preserved as a closed zero-credit pre-decode failure")
    ...
    if not (
        ...
        raise ValueError("F8 r007 inputs and colocated control-copy contract do not match the static review")

    proof = materialization["definition_proof"]
    expected_planes = [-0.075, -0.0675, -0.06, -0.0525, 0.0525, 0.06, 0.0675, 0.075]
    if not (
        proof.get("boundary_particles_expected") == 4096
        and proof.get("fluid_particles_expected") == 6656
        and proof.get("total_particles_expected") == 10752
        and proof.get("boundary_z_planes_expected_m") == expected_planes
        and proof.get("normal_interface_expected_z_m") == [-0.04875, 0.04875]
        and proof.get("domain_z_m") == [-0.09, 0.09]
    ):
        raise ValueError("F8 r007 materialized Definition proof differs from the frozen two-dp margin design")
    cell_count = 32 * 16
    plane_counts = {
        "-0.0750": cell_count, "-0.0675": cell_count, "-0.0600": cell_count, "-0.0525": cell_count,
        "0.0525": cell_count, "0.0600": cell_count, "0.0675": cell_count, "0.0750": cell_count,
    }
    return {
        "definition": definition, "control": control, "control_copy": copy,
        "constantsdef": proof["constantsdef"],
        "expected_wall_plane_particle_counts": plane_counts,
        ...
        "native_inward_normal_z_component_minimum_m": 0.001875,
        "fluid_mass_kg_expected": 2.8079999999999994,
...
def build_authorization() -> dict[str, Any]:
    ...
    return {
        "schema": "core.cfd.f8.r007_cpu_native_preflight_authorization.v1",
        "scope_id": SCOPE,
        "status": "authorized_for_exactly_one_r007_cpu_native_preflight",
        "authorization_basis": "user-wide exploration authorization plus hash-closed r007 review and materialization; r006 is retained as a failed no-decode predecessor",
        "qualification_claim": "none",
        "qualification_credit": 0,
        "closed_prior_scopes": load(REVIEW)["closed_prior_execution_scopes"],
        "retained_previous_failure": {
            "scope_id": "F8_OSCILLATORY_PRESSURE_CHANNEL_WOMERSLEY_R006",
            "status": prior["previous_failure"]["status"],
            "native_decode_invoked": False,
            "same_input_retry_forbidden": True,
            "prior_output_reuse_forbidden": True,
        },
        "single_input": {
            "definition": prior["definition"], "control": prior["control"],
            "r001_r002_r003_r004_r005_r006_inputs_or_outputs_reused": False,
        ...
        "hard_gates": {
            "input_constantsdef_matches_r007_contract": True,
        ...
        "bindings": [
            binding(REVIEW, "passed immutable F8 r007 static design review"),
            binding(MATERIALIZATION, "immutable F8 r007 input materialization receipt"),
            binding(inputs.DEFINITION_TARGET, "only permitted r007 Definition"),
            binding(inputs.CONTROL_TARGET, "only permitted r007 colocated acceleration control"),
            binding(R006_PREFLIGHT, "closed r006 generated-geometry failure receipt"),
            binding(R006_AUTHORIZATION, "immutable one-shot r006 authorization, including no-retry policy"),
            binding(design.R006_BOUND, "r006 seven-plane Bound.vtk failure evidence"),
            binding(design.R006_HDP, "r006 passing half-dp hdp interface"),
            binding(GENCASE, "official CPU GenCase binary"),
            binding(DECODER, "pinned native BI4 decoder"),
            binding(Path("scripts/f8_r007_cpu_native_preflight_authorization_v1.py"), "r007 one-shot authorization builder"),
            binding(RUNNER, "r007 argv-only admission runner"),
            binding(EXECUTOR, "r007 exactly-once preflight executor"),
            binding(SHARED_AUDITOR, "shared generated/native geometry audit helpers"),
            binding(SHARED_AUDITOR_TESTS, "tests covering shared four-layer geometry and normal gates"),
            binding(NATIVE_HELPER, "pinned native BI4 array decoder and CPU environment"),
            binding(Path("tests/test_f8_r007_cpu_native_preflight_v1.py"), "r007 admission and fail-closed tests"),
        ],
    }
...
def write_authorization(path: Path = OUTPUT) -> dict[str, Any]:
    ...
    if target.exists():
        raise FileExistsError(f"refusing to overwrite immutable F8 r007 authorization: {target}")
