#!/usr/bin/env python3
"""Execute F8 r005's exactly-once CPU/native preflight and retain all outcomes."""
...
OUTPUT = ROOT
SCOPE = design.SCOPE
SCHEMA = "core.cfd.f8.r005_cpu_native_preflight.v1"
...
def ref(path: Path, role: str) -> dict[str, Any] | None:
    ...
    return {
        "path": display_path, "sha256": sha256(path),
...
def write_json(path: Path, value: dict[str, Any]) -> None:
    if path.exists():
        raise FileExistsError(f"immutable F8 r005 receipt already exists: {path}")
...
def plane_counts(values: np.ndarray) -> dict[str, int]:
    counts = Counter(f"{round(float(value), 7):.4f}" for value in values)
    return dict(sorted(counts.items()))


def generated_checks(generated: Path, bound_vtk: Path, authorization: dict[str, Any]) -> dict[str, Any]:
    ...
    points = design.vtk_binary_points(bound_vtk)
    zcounts = plane_counts(np.asarray([point[2] for point in points], dtype=np.float64))
    ...
    checks = {
        ...
        "generated_wall_vtk_point_count_matches_fixed": len(points) == fixed_count,
        "generated_z_wall_plane_particle_counts_exact": zcounts == expected_planes,
    ...
    return {
        ...
        "generated_geometry": {
            "bound_vtk_points": len(points), "wall_plane_particle_counts": zcounts,
...
def native_checks(generated: Path, base: Path, authorization: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_planes = hard["native_boundary_z_wall_plane_particle_counts"]
    boundary_planes = plane_counts(native_data["position"][:boundary, 2]) if boundary else {}
    ...
    checks = {
        "native_boundary_particle_count_positive": boundary > 0,
        "native_boundary_particles_exact": boundary == int(hard["native_boundary_particles_exact"]),
        "native_boundary_count_matches_generated": boundary == generated_fixed_count,
        "native_boundary_z_wall_plane_particle_counts_exact": boundary_planes == expected_planes,
        ...
        "native_boundary_normal_file_size_exact": normal_file.is_file() and normal_file.stat().st_size == boundary * 3 * np.dtype(np.float32).itemsize,
        "native_boundary_normals_finite_and_nonzero": bool(len(normals) and np.isfinite(normals).all() and np.isfinite(normal_norms).all() and np.all(normal_norms > 0)),
        ...
        "native_arrays_finite": bool(np.isfinite(native_data["position"]).all() and np.isfinite(native_data["velocity"]).all() and np.isfinite(native_data["density"]).all()),
...
def run_once(output: Path = OUTPUT) -> dict[str, Any]:
    ...
    write_json(lock, {
        "schema": "core.cfd.f8.r005_cpu_native_preflight_lock.v1",
        "created_at_utc": stamp(),
        "authorization": ref(LAB / plan["authorization"], "r005 one-shot authorization"),
    ...
    receipt: dict[str, Any] = {
        ...
        "authorization": ref(LAB / plan["authorization"], "r005 one-shot authorization"),
        "input": {
            "definition": ref(Path(plan["input"]["definition"]), "r005 Definition"),
            "control": ref(Path(plan["input"]["control"]), "r005 colocated acceleration control"),
        },
        ...
        "failure_policy": "Any failed hard gate closes r005 with zero credit; no retry or r001--r004 output reuse.",
    ...
    try:
        ...
        if not constant_audit["pass"]:
            receipt.update({"status": "input_constantsdef_failed_hard_audit", "failure": "r005 constantsdef contract did not match before GenCase"})
        else:
            ...
            if result.returncode != 0 or not generated.is_file() or not bi4.is_file() or not bound_vtk.is_file() or not copied_matches:
                receipt.update({"status": "cpu_gencase_or_control_copy_failed_hard_audit", "failure": "GenCase/XML/BI4/Bound.vtk/control-copy hard gate failed"})
            else:
                generated_audit = generated_checks(generated, bound_vtk, authorization)
                receipt["generated_geometry_audit"] = generated_audit
                if not generated_audit["pass"]:
                    receipt.update({"status": "generated_geometry_failed_hard_audit", "failure": "generated boundary/fluid geometry gates failed before native decode"})
                else:
                    ...
                    if result.returncode != 0:
                        ...
                        if not audit["pass"]:
                            receipt["failure"] = "one or more frozen r005 native-initial hard gates failed"
