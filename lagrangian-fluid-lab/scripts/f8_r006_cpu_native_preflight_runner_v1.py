#!/usr/bin/env python3
"""Validate F8 r006's sealed admission and construct argv without execution."""
...
def validate_authorization(value: dict[str, Any]) -> None:
    expected_schema = "core.cfd.f8.r006_cpu_native_preflight_authorization.v1"
    if not (
        value.get("schema") == expected_schema
        and value.get("scope_id") == SCOPE
        and value.get("status") == "authorized_for_exactly_one_r006_cpu_native_preflight"
        and value.get("qualification_credit") == 0
    ):
        raise PermissionError("F8 r006 authorization is not an active zero-credit one-shot admission")
    ...
    if not (
        ...
        raise PermissionError("F8 r006 authorization opens a prohibited execution surface or lacks fail-closed policy")
    for item in value.get("bindings", []):
        ...
        if not path.is_file() or item.get("sha256") != sha256(path) or item.get("bytes") != path.stat().st_size:
            raise ValueError(f"F8 r006 authorization binding changed: {path}")
...
def build_execution_plan(output: Path = OUTPUT) -> dict[str, Any]:
    ...
    if output.exists():
        raise RuntimeError("F8 r006 output already exists; retry or reuse is forbidden")
    ...
    return {
        ...
        "generated": {
            ...
            "fluid_vtk": str(prefix) + "_Fluid.vtk",
            "hdp_actual_vtk": str(prefix) + "_hdp_Actual.vtk",
