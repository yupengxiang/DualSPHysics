...
SCHEMA = "core.material.f3.native_volume_mls.v2"
BACKEND = "f3_native_volume_mls_wendland_mass_density_v2"
...
CHECKPOINT_SCHEMA = "core.material.f3.native_volume_mls.checkpoint.v2"
CHECKPOINT_FIELDS = (
...
def _trace_binding(source: Path, provider: F3ReferenceProvider, tracer: F3NativeVolumeMLS,
    ...
    return {
        ...
        "frame_policy": "current selected native frame only; all RK stages for interval i use selected frame i",
...
def _summary(handle, binding: dict, labels: np.ndarray, provider: F3ReferenceProvider,
    ...
    return {
        ...
        "source_window": {
            ...
            "native_frame_policy": "selected native source frames sequentially; no interpolation-created source frame",
...
def audit_source(source: str | Path, prepared: str | Path, *, dp_m: float | None = None,
    ...
    with h5py.File(source, "r") as handle:
        ...
        sample_positions = (0, len(selected_indices) // 2, len(selected_indices) - 1)
        mass_sums = [float(np.asarray(handle["mass"][int(selected_indices[i])], dtype=np.float64)[fluid0].sum())
                     for i in sample_positions]
    seeds = seeds_f3(512)
    labels = source_labels(seeds)
    walls = f3_walls()
    return {
        "schema": "core.material.f3.native_volume_mls.source_preflight.v2",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
...
def run_trace(source: str | Path, output: str | Path, prepared: str | Path, *,
    ...
    with F3ReferenceProvider(source, frame_index_map=frame_index_map) as provider:
