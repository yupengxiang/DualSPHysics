#!/usr/bin/env python3
"""Authorize exactly one CPU solver canary for the F6 v10 observation-axis anchor.

This command performs the second, independent root review after the v10
...
SOURCE_ROOT = LAB / "campaigns/core-v1/cfd/f6-fluid-rigid-body-physical-anchor-observation-axis-v10-root-review-20260921"
PREFLIGHT_ROOT = LAB / "campaigns/core-v1/cfd/f6-fluid-rigid-body-physical-anchor-observation-axis-v10-cpu-native-preflight-20260921"
AUTH_ROOT = LAB / "campaigns/core-v1/cfd/f6-fluid-rigid-body-physical-anchor-observation-axis-v10-solver-canary-20260921"
SCRIPT = Path(__file__).resolve()
RUNTIME_WORKER = LAB / "scripts/f6_physical_anchor_observation_axis_v10_solver_canary.py"
...
DEFINITION = SOURCE_ROOT / "CORE_F6_physical_anchor_single_body_gravity_explicit_body_v10_observation_axis_20260921_Def.xml"
...
ROOT_PREFLIGHT = SOURCE_ROOT / "preflight-static.json"
ROOT_PROPOSAL = SOURCE_ROOT / "proposal.json"
NATIVE_PREFLIGHT = PREFLIGHT_ROOT / "preflight.json"
GENERATED_PREFIX = PREFLIGHT_ROOT / "generated/F6_physical_anchor_observation_axis_v10_cpu_native_preflight_20260921"
...
CASE_ID = "F6_physical_anchor_observation_axis_v10_solver_canary_20260921"
TMAX = 1.5
TOUT = 0.005
EXPECTED_FRAMES = 301
...
def validate_inputs() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    fresh_identity = proposal.get("fresh_identity", {})
    if fresh_identity.get("definition_id") != DEFINITION_ID or proposal.get("T1") is not False:
        raise ValueError("v10 proposal identity or T1 guard is invalid")
    decision = proposal.get("root_review_decision", {})
    if decision.get("cpu_native_preflight_authorized") is not True:
        raise ValueError("v10 proposal does not support a protected canary")
    if decision.get("solver_canary_authorized_now") is not False:
        raise ValueError("the v10 proposal must not self-authorize execution")
    if native.get("status") != "cpu_native_preflight_pass_exact_one" or native.get("preflight_pass") is not True:
        raise ValueError("v10 CPU/native preflight is not an exact-one pass")
    if native.get("qualification_claim") != "none" or native.get("qualification_credit") != 0 or native.get("T1") is not False:
        raise ValueError("v10 CPU/native receipt carries science credit")
    ...
    if any(controls.get(key) for key in ("solver_invoked", "gpu_invoked", "job_created", "matrix_submission")):
        raise ValueError("v10 native preflight already opened a protected execution path")
    if any(controls.get(key) not in (0, False) for key in ("queue_mutation", "registry_mutation", "ledger_mutation")):
        raise ValueError("v10 native preflight mutated protected state")
...
def create() -> dict[str, Any]:
    ...
    review = {
        ...
        "review_id": "F6_physical_anchor_observation_axis_v10_solver_canary_root_review_20260921",
        ...
        "basis": {
            "proposal": ref(ROOT_PROPOSAL, "v10 conditional canary proposal"),
            "root_preflight": ref(ROOT_PREFLIGHT, "v10 root CPU preflight"),
            "native_preflight": ref(NATIVE_PREFLIGHT, "v10 exact-one CPU/native preflight"),
            "definition_contract": ref(CONTRACT, "v10 Definition contract"),
            "event_window": ref(EVENT, "v10 event-window contract"),
            "body_sidecar_schema": ref(SIDECAR_SCHEMA, "v10 body-state/force/torque schema"),
    ...
    job = {
        ...
        "runtime_worker": ref(RUNTIME_WORKER, "single-use CPU solver worker"),
        "inputs": {
            "definition": ref(DEFINITION, "fresh v10 Definition"),
            "definition_contract": ref(CONTRACT, "v10 Definition contract"),
            "sidecar_schema": ref(SIDECAR_SCHEMA, "body-state/force/torque sidecar schema"),
            "event_window": ref(EVENT, "event-window contract"),
            "root_preflight": ref(ROOT_PREFLIGHT, "v10 root preflight"),
            "proposal": ref(ROOT_PROPOSAL, "v10 conditional proposal"),
            "native_preflight": ref(NATIVE_PREFLIGHT, "v10 native preflight"),
