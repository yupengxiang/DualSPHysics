#!/usr/bin/env python3
"""Prepare the F2 full-cup moving catchment mDBC canary.
...
adds four fixed outer-catchment walls, and switches all finite boundaries to
the verified mDBC normal construction.  Preparation is CPU-only; the worker
...
SCHEMA = "core.f2.full_cup_closed_catchment_mdbc.v1"
JOB_SCHEMA = "core.cfd.job.v1"
REVISION_ID = "F2_resting_fill_side_wet_full_cup_closed_catchment_mdbc_v2"
SCOPE_ID = "F2_resting_fill_side_wet_full_cup_closed_catchment_mdbc_x_v1"
CASE_ID = "CORE_F2_resting_fill_side_wet_full_cup_closed_catchment_mdbc_q0p50000000_dp0p007500000000_canary_v2"
JOB_ID = "f2-resting-fill-side-wet-full-cup-closed-catchment-mdbc-canary-v2-001"
DP_M = 0.0075
Q = 0.5
...
def _insert_mdbc_normals(root: ET.Element, cup: dict, receiver: dict, tray: dict, catchment: dict) -> dict:
    """Construct normals from every finite boundary surface in the main case."""
    ...
    if commands is None or main is None or casedef is None:
        raise ValueError("incomplete geometry for mDBC normal construction")
    ...
    entries = (
        (cup, "bottom | left | right | front | back"),
        (receiver, "bottom | left | right | front | back"),
        (tray, "bottom"),
        (catchment, "left | right | front | back"),
    )
    for mk, (box, fill) in enumerate(entries):
        ET.SubElement(normals_list, "setmkbound", {"mk": str(mk)})
        _drawbox(normals_list, fill, list(box["low"]), list(box["size"]), "-0.5")
    ET.SubElement(normals_list, "shapeout", {"file": "hdp"})
    ET.SubElement(normals_list, "resetdraw")
    commands.insert(0, normals_list)
    main.insert(0, ET.Element("runlist", {"name": "GeometryForNormals"}))
    # A source definition from static v3 has no normals node; remove any
    # stale node before adding the exact verified distanceh/svshapes recipe.
    old = casedef.find("normals")
    if old is not None:
        casedef.remove(old)
    ...
    return {
        ...
        "geometry_groups": ["cup", "receiver", "tray_floor", "catchment_side_walls"],
...
def _native_initial(prefix: Path, decoder: Path, expected_fluid: int, static_prefix: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="f2-full-cup-mdbc-preflight-") as folder:
        ...
        normals = np.fromfile(normal_file, dtype=np.float32).reshape(-1, 3) if normal_file.is_file() else np.empty((0, 3), dtype=np.float32)
        ...
        return {
            ...
            "normal_preflight_pass": bool(len(normals) == boundary_count and zero_normals == 0 and np.isfinite(normals).all()),
            # The decoder scratch directory is intentionally ephemeral.  The
            # persistent evidence is the GenCase log plus these counts.
            "normal_file": "decoder scratch BoundNor.bin (not retained)",
        }
...
def prepare_canary(lab: Path, output: Path) -> dict:
    ...
    boundary_recipe = _insert_mdbc_normals(root, static["config"]["cup"], static["config"]["receiver"], static["config"]["tray"], catchment)
    ...
    for key, value in (("Boundary", 2), ("SlipMode", 1), ("TimeMax", TIME_MAX_S), ("TimeOut", OUTPUT_INTERVAL_S),
    ...
    config.update({
        ...
        "case_id": CASE_ID,
        "recipe_id": "F2_resting_fill_side_wet_full_cup_closed_catchment_mdbc_v1",
        "recipe": "mdbc_native",
        "stage": "repair_canary",
        ...
        "physical_case_id": "F2_resting_fill_side_wet_full_cup_closed_catchment_mdbc_geometry_v1",
        "lineage_group_id": SCOPE_ID,
        "control_semantics": "prescribed centered rotation starts at 0.50 s; mDBC wall velocity is supplied by the native mvrotfile",
        "boundary_method": 2,
        "repair_candidate_id": "closed_external_catchment_full_cup_mdbc",
        ...
        "qualification_inheritance": "none; independent full-cup closed-catchment mDBC physical scope",
        "qualification_claim": "none; independent full-cup closed-catchment mDBC moving canary only",
    })
    definition_audit = {
        "source_definition": str(static_definition),
        ...
        "changed_fields": [
            ...
            "Boundary=2 mDBC with explicit cup/receiver/tray/catchment normal geometry",
    ...
    prepared = {
        ...
        "dynamic_canary_preflight": {
            ...
            "mdbc_normals_complete_nonzero": native["normal_preflight_pass"],
            "zero_boundary_normals": native["zero_boundary_normals"],
        ...
        "preflight_pass": bool(
            ...
            and native["normal_preflight_pass"]
...
def make_job(prepared_path: Path, lab: Path, output: Path) -> dict:
    ...
    if not prepared.get("preflight_pass"):
        raise ValueError("F2 full-cup catchment mDBC preflight failed")
    if config.get("repair_candidate_id") != "closed_external_catchment_full_cup_mdbc":
        raise ValueError("prepared input is not the full-cup catchment mDBC candidate")
    ...
    generator = lab / "scripts/f2_full_cup_closed_catchment_mdbc.py"
    spec = {
        "schema": JOB_SCHEMA,
        "job_id": JOB_ID,
        "logical_id": JOB_ID,
        "attempt_role": "full_cup_closed_catchment_mdbc_event_canary",
        "category": "f2_dynamic_full_cup_closed_catchment_mdbc_canary",
        "host": "h200",
        "source_lab": str(lab),
        "cwd": str(lab),
        ...
        "qualification_only": True,
        "split": "qualification_only",
        "qualification_status": "candidate-only; independent full-cup catchment mDBC scope remains unqualified",
        ...
        "input_files": [
            ...
            {"path": str(generator), "sha256": core_cfd.digest(generator)},
        ],
        ...
        "normal_preflight": {
            ...
            "normal_search_distance_h": 3.0,
            "svshapes": True,
            "required_zero_boundary_normals": 0,
        },
        "geometry_contract": {
