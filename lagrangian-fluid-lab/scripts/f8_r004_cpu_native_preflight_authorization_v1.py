#!/usr/bin/env python3
"""Seal the sole F8 r004 CPU/native preflight admission; never run binaries."""
...
from scripts import f8_r004_input_materialization_v1 as inputs
...
RUNNER = Path("scripts/f8_r004_cpu_native_preflight_runner_v1.py")
EXECUTOR = Path("scripts/f8_r004_cpu_native_preflight_execute_v1.py")
...
def bindings_current(receipt: dict[str, Any]) -> bool:
...
def validate_predecessors() -> dict[str, Any]:
    ...
    data, copy = materialization.get("materialized_inputs", {}), materialization.get("control_dependency_copy_proof", {})
    if not (
        review.get("schema") == design.SCHEMA and review.get("scope_id") == SCOPE
        and review.get("status") == "r004_static_design_review_v2_passed_inputs_not_authorized"
        and review.get("static_constraint_gaps") == [] and review.get("qualification_credit") == 0
        and bindings_current(review)
    ):
        raise PermissionError("F8 r004 v2 static review is not current, passed, and zero-credit")
    if not (
        materialization.get("schema") == inputs.SCHEMA and materialization.get("scope_id") == SCOPE
        and materialization.get("status") == "one_time_r004_inputs_materialized_static_only"
        ...
        and data.get("definition_matches_review_precommit") is True and data.get("control_matches_review_precommit") is True
        and bindings_current(materialization)
    ):
        raise PermissionError("F8 r004 materialization is not current, immutable, and hash-closed")
    definition, control = data["definition"], data["control"]
    if not (
        definition["path"] == str(inputs.DEFINITION_TARGET) and control["path"] == str(inputs.CONTROL_TARGET)
        and definition["sha256"] == sha256(LAB / inputs.DEFINITION_TARGET)
        and control["sha256"] == sha256(LAB / inputs.CONTROL_TARGET)
        and copy.get("definition_relative_reference") == inputs.design.v1.control_filename()
        and copy.get("required_generated_copy") == str(design.COPIED_CONTROL)
    ):
        raise ValueError("F8 r004 materialized input/copy binding does not match v2")
    ...
    return {"definition": definition, "control": control, "control_copy": copy,
            "constantsdef": materialization["definition_proof"]["constantsdef"],
            "expected_fluid_particles": count,
...
def build_authorization() -> dict[str, Any]:
    ...
    return {
        "schema": "core.cfd.f8.r004_cpu_native_preflight_authorization.v1", "scope_id": SCOPE,
        "status": "authorized_for_exactly_one_r004_cpu_native_preflight",
        "authorization_basis": "user-wide exploration/execution authorization; immutable r004 v2 review and materialization",
        "qualification_claim": "none", "qualification_credit": 0,
        "closed_prior_scopes": design.load_json(REVIEW)["closed_prior_execution_scopes"],
        "single_input": {"definition": prior["definition"], "control": prior["control"],
                         "r001_r002_r003_input_or_output_reused": False, "constantsdef": prior["constantsdef"]},
        ...
        "hard_gates": {"input_constantsdef_matches_r004_contract": True, "input_hswl_is_explicit": True, "generated_colocated_control_copy_hash_matches": True, "generated_fixed_boundary_count_positive": True, "generated_z_wall_planes_present": True, "native_boundary_particle_count_positive": True, "native_boundary_normal_count_positive": True, "native_boundary_normals_finite_and_nonzero": True, "native_fluid_particle_count_exact": prior["expected_fluid_particles"], "native_fluid_mass_kg_expected": prior["fluid_mass_kg_expected"]},
        ...
        "bindings": [binding(REVIEW, "passed immutable F8 r004 v2 static design review"), binding(MATERIALIZATION, "immutable F8 r004 materialization receipt"), binding(inputs.DEFINITION_TARGET, "only permitted r004 Definition"), binding(inputs.CONTROL_TARGET, "only permitted r004 colocated acceleration control"), binding(GENCASE, "official CPU GenCase binary"), binding(DECODER, "pinned native BI4 decoder"), binding(Path(__file__).relative_to(LAB), "r004 authorization builder"), binding(RUNNER, "r004 argv-only runner"), binding(EXECUTOR, "r004 one-shot executor")],
    }
...
def write_authorization(path: Path = OUTPUT) -> dict[str, Any]:
    ...
    if target.exists():
        raise FileExistsError(f"refusing to overwrite immutable F8 r004 authorization: {target}")
