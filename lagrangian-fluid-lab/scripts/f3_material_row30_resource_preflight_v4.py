...
This v4 receipt is an additive, no-worker refresh. It reuses the reviewed
read-only v2 probes, preserves the v3 one-shot record, and writes a fresh
versioned immutable receipt and lock. It cannot launch or reserve a worker.
"""
...
SCOPE_DIR = LAB / "campaigns/core-v1/material/evidence/f3-material-row30-resource-preflight-v4"
OUTPUT = SCOPE_DIR / "receipt.json"
LOCK = SCOPE_DIR / "one-shot-lock.json"
SCHEMA = "core.material.f3.row30.resource_preflight.v4"
RECORD_ID = "f3-material-row30-resource-preflight-v4"
AUTHORITY = "explicit user authorization: one fresh F3 material row-30 resource/scheduler preflight; no worker launch"
PRIOR_RECEIPT = Path("campaigns/core-v1/material/evidence/f3-material-row30-resource-preflight-v3/receipt.json")
...
def build(
    *,
    ...
    value["supersedes"] = preflight_v2._binding(
        PRIOR_RECEIPT,
        "prior blocked row30 v3 preflight; preserved as historical evidence",
        preflight_v2._read_regular(preflight_v2.LAB / PRIOR_RECEIPT),
    )
    value["bindings"].extend((
        preflight_v2._binding(
            Path("scripts/f3_material_row30_resource_preflight_v4.py"),
            "v4 fresh one-shot preflight wrapper",
            preflight_v2._read_regular(LAB / "scripts/f3_material_row30_resource_preflight_v4.py"),
        ),
        preflight_v2._binding(
            Path("tests/test_f3_material_row30_resource_preflight_v4.py"),
            "v4 preflight contract tests",
            preflight_v2._read_regular(LAB / "tests/test_f3_material_row30_resource_preflight_v4.py"),
        ),
    ))
    return value
...
def write_once(*, scope_dir: Path = SCOPE_DIR) -> dict[str, Any]:
    ...
    if scope.exists() or scope.is_symlink():
        raise FileExistsError(f"F3 row-30 v4 one-shot namespace already exists: {scope}")
