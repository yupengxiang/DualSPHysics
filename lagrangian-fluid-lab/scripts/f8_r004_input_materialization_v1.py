#!/usr/bin/env python3
"""Materialize F8 r004 v2's exact static input pair, without execution.

This consumes only the r004 static-input admission frozen by static-design-
review-v2.  The Definition and its bare-name colocated acceleration table are
created with exclusive writes, then recorded in an immutable receipt.  It does
not run GenCase, native decoding, a solver, GPU, queue, or worker.
"""
...
ROOT = design.ROOT
SCOPE = design.SCOPE
REVIEW = ROOT / "static-design-review-v2/receipt.json"
...
SCHEMA = "core.cfd.f8.r004_input_materialization.v1"
...
def validate_review(review: dict[str, Any], definition: bytes, control: bytes) -> None:
    ...
    namespace = review.get("r004_namespace", {})
    execution = review.get("execution_controls", {})
    if not (
        review.get("schema") == design.SCHEMA
        and review.get("scope_id") == SCOPE
        and review.get("status") == "r004_static_design_review_v2_passed_inputs_not_authorized"
        ...
        and review.get("next_automatic_step", {}).get("kind") == "one-time r004 static input materialization"
    ):
        raise PermissionError("F8 r004 v2 review does not bind this exact one-time input materialization")


def ensure_absent(*targets: Path) -> None:
    existing = [str(target) for target in targets if target.exists()]
    if existing:
        raise FileExistsError("refusing one-time F8 r004 materialization because target already exists: " + ", ".join(existing))
...
def prove_definition(definition: str) -> dict[str, Any]:
    ...
    if not all(all(constants.get(name, {}).get(key) == value for key, value in attrs.items())
        ...
        raise ValueError("materialized F8 r004 Definition lost reviewed constantsdef compatibility fields")
    required_walls = (
    ...
    if not all(fragment in definition for fragment in required_walls):
        raise ValueError("materialized F8 r004 Definition lost reviewed finite z-wall construction")
    if definition.index('name="FiniteNoSlipZWalls"') >= definition.index('name="FullyFilledChannelFluid"'):
        raise ValueError("materialized F8 r004 walls must precede the separate fluid box")
...
def build_receipt(definition: bytes, control: bytes) -> dict[str, Any]:
    ...
    bare_control = design.v1.control_filename()
    if f'value="{bare_control}"' not in definition_text or "acceleration/" in definition_text:
        raise ValueError("materialized F8 r004 Definition lost reviewed bare colocated control reference")
    if not control_text.startswith("#Time;LinearAccX;") or not control_text.endswith(";0;0;0;0;0\n"):
        raise ValueError("materialized F8 r004 acceleration table lost reviewed format or endpoint")
    return {
        "schema": SCHEMA, "scope_id": SCOPE,
        "status": "one_time_r004_inputs_materialized_static_only",
        "qualification_claim": "none", "qualification_credit": 0,
        "retained_static_rejection": {"scope": SCOPE, "review": str(design.V1_RECEIPT),
                                       "status": "r004_static_design_review_failed", "execution_performed": False},
        "closed_prior_execution_scopes": design.load_json(REVIEW)["closed_prior_execution_scopes"],
        "authorization": {"source": str(REVIEW), "review_schema": design.SCHEMA, "consumed": True,
                          "overwrite_or_reuse_allowed": False, "definition_target": str(DEFINITION_TARGET),
                          "control_target": str(CONTROL_TARGET)},
        "materialized_inputs": {
            "definition": binding(DEFINITION_TARGET, "precommitted F8 r004 Definition XML"),
            "control": binding(CONTROL_TARGET, "precommitted F8 r004 colocated acceleration CSV"),
        ...
        "control_dependency_copy_proof": {
            "definition_relative_reference": bare_control, "source_control": str(CONTROL_TARGET),
            "source_is_colocated_with_definition": True, "required_generated_copy": str(design.COPIED_CONTROL),
            "required_copy_hash": sha256_bytes(control),
            "r001_r002_r003_and_r004v1_paths_explicitly_forbidden": [
                "acceleration/F8_OPC_q0p500_acceleration.csv", "acceleration/F8_OPC_q0p500_r002_acceleration.csv",
                "acceleration/F8_OPC_q0p500_r003_acceleration.csv", "r004 static-design-review-v1 output",
            ],
        },
        "execution_controls": {
        ...
        "next_automatic_step": {"kind": "one-time F8 r004 CPU-native-preflight authorization and execution",
                                  "requires": ["a new immutable authorization binding this scope, v2 review, and both materialized hashes", "a new isolated r004 output namespace", "at most one GenCase invocation followed only by declared static/native gates"]},
        "bindings": [
            binding(REVIEW, "immutable F8 r004 v2 static design review"),
            binding(design.V1_RECEIPT, "immutable rejected F8 r004 v1 static review"),
            binding(Path("scripts/f8_r004_static_design_review_v2.py"), "reviewed r004 v2 in-memory input renderer"),
            binding(Path("scripts/f8_r004_input_materialization_v1.py"), "exclusive-create r004 input materializer"),
            binding(DEFINITION_TARGET, "materialized Definition"), binding(CONTROL_TARGET, "materialized colocated acceleration table"),
        ],
    }
...
def write_materialization(
    ...
    if (definition_target, control_target, receipt_target) != registered:
        raise ValueError("F8 r004 materializer only permits its registered targets")
