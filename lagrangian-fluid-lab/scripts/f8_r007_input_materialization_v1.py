#!/usr/bin/env python3
"""Materialize the precommitted F8 r007 Definition and colocated control CSV once."""
...
SCHEMA = "core.cfd.f8.r007_input_materialization.v1"
...
def validate_review(review: dict[str, Any], definition: bytes, control: bytes) -> None:
    ...
    namespace = review.get("r007_namespace", {})
    if not (
        ...
        raise PermissionError("F8 r007 static review does not bind this exact one-time materialization")
...
def prove_definition(definition: str) -> dict[str, Any]:
    ...
    base_wall, outer_wall = half_height + dp, half_height + 4.0 * dp
    ...
    if any(any(constants.get(name, {}).get(key) != value for key, value in attrs.items())
           for name, attrs in expected_constants.items()):
        raise ValueError("r007 Definition lost reviewed constantsdef compatibility fields")
    ...
    if geometry is None or domain is None or normal_list is None or mainlist is None or normals is None:
        raise ValueError("r007 Definition lost its geometry or native-normal sections")
    point_min, point_max = float(domain.find("./pointmin").get("z")), float(domain.find("./pointmax").get("z"))
    if not (math.isclose(point_min, -0.09, abs_tol=1e-14) and math.isclose(point_max, 0.09, abs_tol=1e-14)
            and math.isclose((point_max - outer_wall) / dp, 2.0, abs_tol=1e-12)):
        raise ValueError("r007 symmetric computational domain does not provide two dp beyond four wall layers")
    ...
    for wall in (normal_wall, particle_wall):
        if (wall is None or wall.findtext("./boxfill") != "top | bottom"
                or not math.isclose(float(wall.find("./point").get("z")), -base_wall, abs_tol=1e-14)
                or not math.isclose(float(wall.find("./size").get("z")), 2.0 * base_wall, abs_tol=1e-14)):
            raise ValueError("r007 finite wall faces changed from the reviewed r006 geometry")
    if (normal_wall.find("./layers[@vdp='-0.5']") is None
            or particle_wall.find("./layers[@vdp='0,1,2,3']") is None):
        raise ValueError("r007 must preserve half-dp normal interface and four outward boundary layers")
    if (fluid is None or fluid.findtext("./boxfill") != "solid"
        ...
        raise ValueError("r007 fully filled fluid interval changed")
    if (normals.find("./geometryfile").get("file") != "[CaseName]_hdp_Actual.vtk"
        ...
        raise ValueError("r007 native normal settings changed from the successful geometry precedent")
    if f'value="{design.control_filename()}"' not in definition or "acceleration/" in definition:
        raise ValueError("r007 Definition lost the colocated bare-filename control reference")

    planes = [-outer_wall, -outer_wall + dp, -outer_wall + 2 * dp, -base_wall,
              base_wall, base_wall + dp, base_wall + 2 * dp, outer_wall]
    return {
        "constantsdef": expected_constants,
        "domain_z_m": [point_min, point_max],
        "domain_clearance_beyond_outer_layers_dp": 2.0,
        "base_wall_faces_z_m": [-base_wall, base_wall],
        "normal_interface_expected_z_m": [-base_wall + dp / 2, base_wall - dp / 2],
        "boundary_layers_vdp": [0, 1, 2, 3],
        "boundary_z_planes_expected_m": planes,
...
def build_receipt(definition: bytes, control: bytes) -> dict[str, Any]:
    ...
    if control != (LAB / design.R006_CONTROL).read_bytes():
        raise ValueError("r007 acceleration values changed from the exact r006 positive-control table")
    if not text.startswith("#Time;LinearAccX;") or not text.endswith(";0;0;0;0;0\n"):
        raise ValueError("r007 acceleration CSV format or endpoint changed")
    review = load_json(REVIEW)
    return {
        "schema": SCHEMA,
        "scope_id": SCOPE,
        "status": "one_time_r007_inputs_materialized_static_only",
        "qualification_claim": "none",
        "qualification_credit": 0,
        "retained_r006_failure": {
            "receipt": str(design.R006_PREFLIGHT),
            "status": load_json(design.R006_PREFLIGHT)["status"],
        ...
        "materialized_inputs": {
            "definition": binding(DEFINITION_TARGET, "precommitted F8 r007 Definition XML"),
            "control": binding(CONTROL_TARGET, "precommitted F8 r007 colocated acceleration CSV"),
            "definition_matches_review_precommit": True,
            "control_matches_review_precommit": True,
        },
        "definition_proof": prove_definition(definition.decode("utf-8")),
        "control_dependency_copy_proof": {
            "definition_relative_reference": design.control_filename(),
            "source_control": str(CONTROL_TARGET), "source_is_colocated_with_definition": True,
        ...
        "next_automatic_step": {
            "kind": "one-time F8 r007 CPU-native-preflight authorization and execution",
            "requires": [
                "new immutable authorization binding r007 review, materialization, tools, and test hashes",
                "one isolated r007 output namespace and invocation lock",
                "one GenCase followed by frozen geometry gates before at most one native decode",
            ],
        },
        "bindings": [
            binding(REVIEW, "immutable passed F8 r007 static design review"),
            binding(Path("scripts/f8_r007_static_design_review_v1.py"), "r007 precommitted Definition renderer"),
            binding(Path("scripts/f8_r007_input_materialization_v1.py"), "exclusive-create r007 input materializer"),
            binding(Path("tests/test_f8_r007_input_materialization_v1.py"), "r007 materialization contract tests"),
            binding(DEFINITION_TARGET, "materialized r007 Definition"),
            binding(CONTROL_TARGET, "materialized r007 colocated control CSV"),
        ],
    }
...
def write_materialization(
    ...
    if (definition_target, control_target, receipt_target) != registered:
        raise ValueError("F8 r007 materializer only permits its registered targets")
    ...
    if existing:
        raise FileExistsError("refusing one-time F8 r007 materialization because target exists: " + ", ".join(existing))
