#!/usr/bin/env python3
"""Prepare and run the exact F3 CFD source cases for the Core campaign.
...
``prepare`` and ``make-job`` are CPU/metadata operations.  ``run`` is the
runtime worker entrypoint used by ``core_runtime``.  The coordinator assigns
one CUDA device through ``CUDA_VISIBLE_DEVICES``; the solver therefore always
receives ``-gpu:0`` inside that remapped device namespace.  No GPU is selected
by this module and no campaign ledger is touched.
"""
...
def _rewrite_xml(path: Path, *, dp_m: float, output_interval_s: float) -> None:
    tree = ET.parse(path)
    root = tree.getroot()
    ...
    for key, value in (("TimeMax", TIME_MAX_S), ("TimeOut", output_interval_s), ("CoefDtMin", COEF_DT_MIN)):
...
def _audit_hdf5(prepared: dict[str, Any], hdf5: Path, output: Path) -> dict[str, Any]:
    required = {"particle_id", "particle_zone", "source_label_initial_mk", "time", "position", "velocity",
                "density", "mass", "pressure", "valid", "type", "mk"}
    expected_frames = int(prepared["case"]["expected_native_frames"])
    # The converted material/source trajectory intentionally contains the
    # fluid block only.  Boundary particles remain in the raw solver output
    # and are not part of the material reference H5.
    expected_particles = int(prepared["case"]["fluid_particles"])
    with core_cfd.h5py.File(hdf5, "r") as h:
        datasets = set(h.keys())
        missing = sorted(required - datasets)
        frames = int(h["time"].shape[0]) if "time" in h else 0
        particles = int(h["particle_id"].shape[0]) if "particle_id" in h else 0
        times = np.asarray(h["time"][:], dtype=np.float64) if "time" in h else np.empty(0)
        ids = np.asarray(h["particle_id"][:]) if "particle_id" in h else np.empty(0)
        valid = np.asarray(h["valid"][:], dtype=bool) if "valid" in h else np.empty((0, 0), dtype=bool)
        mass = np.asarray(h["mass"][:], dtype=np.float64) if "mass" in h else np.empty((0, 0))
        finite = bool(all(np.isfinite(h[name][:]).all() for name in ("position", "velocity", "density", "mass", "pressure"))) if not missing else False
        unique_ids = bool(len(ids) == len(np.unique(ids))) if "particle_id" in h else False
        valid_complete = bool(valid.shape == (frames, particles) and valid.all()) if "valid" in h else False
        time_strict = bool(len(times) >= 2 and np.isfinite(times).all() and np.all(np.diff(times) > 0))
        full_window = bool(len(times) == expected_frames and time_strict and times[0] >= -1e-8 and times[-1] >= TIME_MAX_S - 1e-5)
        initial_mass = mass[0] if mass.ndim == 2 and len(mass) else np.empty(0)
        mass_drift = float(np.max(np.abs(mass.sum(axis=1) - initial_mass.sum())) / max(abs(initial_mass.sum()), 1e-30)) if len(mass) else math.inf
        source_mass = {
            str(int(label)): float(initial_mass[np.asarray(h["source_label_initial_mk"][:]) == label].sum())
            for label in np.unique(h["source_label_initial_mk"][:])
        } if "source_label_initial_mk" in h and len(initial_mass) else {}
    report = {
        "schema": "core.f3.cfd.native_volume_mls.source_audit.v1",
        ...
        "all_fluid_frames_valid": valid_complete,
        "strictly_increasing_time": time_strict,
        "time_start_s": None if not len(times) else float(times[0]),
        "time_end_s": None if not len(times) else float(times[-1]),
        "full_window_pass": full_window,
        "mass_drift_relative_max": mass_drift,
        "source_mass_initial_kg_by_mk": source_mass,
        "mass_closure_pass": bool(math.isfinite(mass_drift) and mass_drift <= 1e-8),
        "hard_integrity_pass": bool(not missing and frames == expected_frames and particles == expected_particles and finite and unique_ids and valid_complete and full_window and math.isfinite(mass_drift) and mass_drift <= 1e-8),
...
def run_case(prepared_path: Path, output: Path, *, lab: Path = LAB) -> dict[str, Any]:
    ...
    audit = _audit_hdf5(prepared, output / "trajectory.h5", output)
    result = {
