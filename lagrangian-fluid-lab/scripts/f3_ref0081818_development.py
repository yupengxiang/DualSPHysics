...
entry point.  It deliberately requires a *passed* ref0081818 qualification
...
from scripts import f3_ref0081818_score as revision_score
...
REVISION_RECIPE = "F3_CELL3_NS_visco1_native_nopen_revision075_ref0081818"
REVISION_GATE = "F3-075-REF0081818-GATE.json"
DEVELOPMENT_MANIFEST = "F3-DEVELOPMENT-CANDIDATES.json"
REGISTRY = "F3-075-REF0081818-DEVELOPMENT-REGISTRY.json"
GATE_SCHEMA = "f3.revision075.ref0081818.gate.v1"
REGISTRY_SCHEMA = "f3.ref0081818.development_registry.v1"
GPU_IDS = (4, 5, 6, 7)
...
def verify_revision_gate() -> dict:
    ...
    if (gate.get("schema") != GATE_SCHEMA
        ...
        raise ValueError("passed ref0081818 qualification gate with development opt-in is required")
    ...
    if gate.get("coordinate_frame") != "fixed tank computational coordinates":
    ...
    if (score.get("schema") != "f3.revision075.ref0081818.stage_scores.v1"
            or score.get("status") != "passed"
            or score.get("recipe_id") != REVISION_RECIPE
            or score.get("production_source_case_id") != gate.get("production_source_case_id")
            or score.get("reference_resolutions_m") != [revision_score.preparation.DP, 0.0075, 0.006]):
        raise ValueError("ref0081818 scoring report is not a passed bound report")
    if gate.get("scoring_evidence_sha256") != q2.sha256(score_path):
        raise ValueError("ref0081818 scoring report hash changed")
    score_evidence = score.get("evidence_sha256")
    if not isinstance(score_evidence, dict) or not score_evidence:
        raise ValueError("ref0081818 scoring report has no bound evidence")
    for relative, digest in score_evidence.items():
        if q2.sha256(_path(relative)) != digest:
            raise ValueError("ref0081818 scoring evidence changed: " + relative)
    ...
    for path in (source_record, source_solver, source_audit):
        if _relative(path) not in score_evidence:
...
def _context(*, register: bool) -> dict:
    ...
    if registry_path.exists():
        ...
        registry = {
            ...
            "scope": "independent_development_after_f3_ref0081818_qualification",
...
def _fields(row: dict, context: dict, target: Path) -> dict:
    ...
    return dict(
        ...
        background_id="plain", phase="F3_ref0081818_development",
        ...
        comparison_scope="Independent physical forcing case inside the qualified F3 ref0081818 domain; public development only",
        input_repair="Reuse qualified ref0081818 native initial-state/solver inputs with exact registered independent CSV",
    )
