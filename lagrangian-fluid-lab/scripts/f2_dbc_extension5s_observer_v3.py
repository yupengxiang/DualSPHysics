#!/usr/bin/env python3
"""Independent read-only metadata correction for the F2 DBC extension.

The v2 report accidentally recorded the initial fluid box as the physical cup
contract.  This revision binds cup classification to ``config.cup`` and keeps
the side-wet initial fluid continuum box as a separate contract.  It also
renames the 1.01004 s outside classification as an observer threshold; it does
not assert physical spill.  The completed trajectory is opened read-only and
the solver is never rerun.
"""
...
SCHEMA = "core.f2.dbc_duration.observer_reaudit.v3"
OBSERVER_REVISION = "F2_geometry_aware_observer_v3_cup_metadata_and_outside_semantics"
...
def _initial_fluid_box(config: dict[str, Any]) -> dict[str, Any]:
    boxes = config.get("fluid_boxes", [])
    if len(boxes) != 1:
        raise ValueError("v3 full-cup observer expects one continuous fluid box")
    box = boxes[0]
    ...
    return {
        "low": low.tolist(),
        "high": high.tolist(),
        "source": "config fluid_boxes physical continuum faces",
        "contract_role": "initial fluid continuum, not cup classification",
    }
...
def _summarize(trajectory: Path, prepared: Path, original_observations: dict[str, Any]) -> dict[str, Any]:
    ...
    physical_cup = {
        "low": cup_low.tolist(),
        "high": cup_high.tolist(),
        "source": "config cup physical interior faces",
        "contract_role": "moving cup classification used by this re-audit",
    }
    initial_fluid = _initial_fluid_box(config)
    ...
    return {
        ...
        "contract_change": {
            ...
            "metadata_correction": "v2 physical_cup_contract incorrectly pointed to config fluid_boxes; v3 separates the actual cup from the initial fluid continuum",
        },
        "physical_cup_contract": physical_cup,
        "initial_fluid_continuum_contract": initial_fluid,
        ...
        "revised_event_times_s": {
            ...
            "outside_observer_threshold": float(times[spill_indices[0]]) if len(spill_indices) else None,
            "spill_or_escape": None,
        ...
        "outside_observer_threshold_semantics": {
            "time_key": "revised_event_times_s.outside_observer_threshold",
            "meaning": "first frame whose fixed-mass observer classification leaves cup/receiver/tray boxes",
            "physical_spill_verified": False,
            "physical_spill_claim": "none; endpoint/chord and destination physics require a separate geometry-aware audit",
        },
        "revised_tail": {
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "outside_observer_threshold": report["revised_event_times_s"]["outside_observer_threshold"],
        "physical_spill_verified": report["outside_observer_threshold_semantics"]["physical_spill_verified"],
