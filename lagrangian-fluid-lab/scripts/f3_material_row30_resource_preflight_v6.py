...
This additive v6 wrapper preserves the consumed v1-v5 history and delegates
...
SCOPE_DIR = LAB / "campaigns/core-v1/material/evidence/f3-material-row30-resource-preflight-v6"
OUTPUT = SCOPE_DIR / "receipt.json"
LOCK = SCOPE_DIR / "one-shot-lock.json"
SCHEMA = "core.material.f3.row30.resource_preflight.v6"
RECORD_ID = "f3-material-row30-resource-preflight-v6"
AUTHORITY = "explicit user authorization: one new F3 material row-30 resource/scheduler preflight; no worker launch"
PRIOR_RECEIPT = Path("campaigns/core-v1/material/evidence/f3-material-row30-resource-preflight-v5/receipt.json")
...
def build(*, environment_probe=None, process_probe=None, now: datetime | None = None) -> dict[str, Any]:
    ...
    value["supersedes"] = preflight_v2._binding(
        PRIOR_RECEIPT,
        "prior blocked row30 v5 preflight; preserved as historical evidence",
        preflight_v2._read_regular(LAB / PRIOR_RECEIPT),
    )
    value["bindings"].extend((
        preflight_v2._binding(
            Path("scripts/f3_material_row30_resource_preflight_v6.py"),
            "v6 fresh one-shot preflight wrapper",
            preflight_v2._read_regular(LAB / "scripts/f3_material_row30_resource_preflight_v6.py"),
        ),
        preflight_v2._binding(
            Path("tests/test_f3_material_row30_resource_preflight_v6.py"),
            "v6 preflight contract tests",
            preflight_v2._read_regular(LAB / "tests/test_f3_material_row30_resource_preflight_v6.py"),
        ),
    ))
    return value
...
def write_once(*, scope_dir: Path = SCOPE_DIR) -> dict[str, Any]:
    ...
    if scope.exists() or scope.is_symlink():
        raise FileExistsError(f"F3 row-30 v6 one-shot namespace already exists: {scope}")
