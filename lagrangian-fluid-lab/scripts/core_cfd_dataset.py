...
F4_PRODUCTION_COLLECTION_SCHEMA = "core.f4.tallwall120.production_collection.v1"
SOURCE_SCHEMAS = {
    SCHEMA,
    F4_PRODUCTION_COLLECTION_SCHEMA,
    "core.cfd.v1",
...
def adapt_manifest(manifest, data_root, *, require_existing=True):
    ...
    if schema in {DATASET_SCHEMA, COMPACT_SCHEMA}:
        ...
        return payload
    if schema == F4_PRODUCTION_COLLECTION_SCHEMA:
        # The collection report retains the complete 32-case denominator and
        # failed/pending audit rows.  Only its explicitly materialized reader
        # view may enter CoreDataset; a partial collection therefore fails
        # closed instead of being mistaken for a training manifest.
        reader = payload.get("reader_manifest")
        if not isinstance(reader, dict):
            raise ValueError("F4 production collection has no reader manifest")
        validate_manifest(reader)
        return reader
...
__all__ = ["SCHEMA", "F4_PRODUCTION_COLLECTION_SCHEMA", "SOURCE_SCHEMAS", "CoreCFDDataset", "CFDDataset",
