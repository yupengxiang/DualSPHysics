#!/usr/bin/env python3
"""Validate F8 r003's sealed admission and construct argv; never run it."""
...
def load_authorization(path: Path = LAB / AUTHORIZATION) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError("F8 r003 authorization must be a JSON object")
    return value


def validate_authorization(value: dict[str, Any]) -> None:
    if not (value.get("schema") == "core.cfd.f8.r003_cpu_native_preflight_authorization.v1" and value.get("scope_id") == SCOPE and value.get("status") == "authorized_for_exactly_one_r003_cpu_native_preflight" and value.get("qualification_credit") == 0):
        raise PermissionError("F8 r003 authorization is not an active zero-credit one-shot admission")
    ...
    if not (permissions.get("cpu_gencase") is True and permissions.get("native_decode") is True and all(permissions.get(key) is False for key in ("solver", "gpu", "queue", "worker", "registry", "ledger", "training", "qualification")) and all(permissions.get(key) == 0 for key in ("queue_mutation", "registry_mutation", "ledger_mutation", "denominator_mutation"))):
        raise PermissionError("F8 r003 authorization opens a prohibited execution surface")
    for item in value.get("bindings", []):
        ...
        if not path.is_file() or item.get("sha256") != sha256(path) or item.get("bytes") != path.stat().st_size:
            raise ValueError(f"F8 r003 authorization binding changed: {path}")
...
def build_execution_plan(output: Path = OUTPUT) -> dict[str, Any]:
    ...
    if output != OUTPUT.resolve():
        raise ValueError("only the registered F8 r003 output namespace is permitted")
    if output.exists():
        raise RuntimeError("F8 r003 output already exists; retry or reuse is forbidden")
