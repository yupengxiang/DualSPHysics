#!/usr/bin/env python3
"""Execute the single root-authorized CPU solver canary for F6 v9.

The worker is deliberately single-use.  It validates every hash-bound input,
...
JOB = LAB / "campaigns/core-v1/cfd/f6-fluid-rigid-body-physical-anchor-solver-canary-v9-20260921/job.json"
REVIEW = LAB / "campaigns/core-v1/cfd/f6-fluid-rigid-body-physical-anchor-solver-canary-v9-20260921/root-review.json"
DEFAULT_OUTPUT = LAB / "campaigns/core-v1/cfd/f6-fluid-rigid-body-physical-anchor-solver-canary-v9-20260921/attempt-001"
...
def verify_authorization() -> tuple[dict[str, Any], dict[str, Any]]:
    job = load(JOB)
    review = load(REVIEW)
    if job.get("schema") != "core.f6.physical_anchor.protected_solver_canary_job.v1":
    ...
    if review.get("schema") != "core.f6.physical_anchor.protected_solver_canary_root_review.v1":
...
def make_sidecar(rows: list[dict[str, float]], frame_audits: list[dict[str, Any]], contract: dict[str, Any], output: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for row, frame in zip(rows, frame_audits):
        ...
        status = "settle_hold" if times[len(sidecar["event_status"])] >= 1.0 else "fluid_contact" if contact else "pre_contact"
    ...
    hold_rows = [index for index, value in enumerate(times) if 1.0 - CADENCE_TOLERANCE_S <= value <= 1.5 + CADENCE_TOLERANCE_S]
    checks = {
        ...
        "sidecar_cadence_exact": bool(len(times) == EXPECTED_FRAMES and np.allclose(np.diff(times), TOUT, rtol=0.0, atol=CADENCE_TOLERANCE_S)),
        ...
        "contact_time_in_predicted_window": contact_time is not None and contact_window[0] - CADENCE_TOLERANCE_S <= contact_time <= contact_window[1] + CADENCE_TOLERANCE_S,
        ...
        "settle_hold_complete": len(hold_rows) == 101,
        "settle_hold_uncensored": bool(hold_rows and hold_rows[0] == 200 and hold_rows[-1] == 300),
    }
    sidecar["audit"] = {"contact_time_s": contact_time, "predicted_contact_window_s": contact_window, "checks": checks}
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    command = [str(binary), "-tmax:1.5", "-tout:0.005", str(prefix), str(solver_dir)]
    ...
    with tempfile.TemporaryDirectory(prefix="f6-canary-native-") as temp_dir:
    ...
    native_checks = {
        ...
        "native_time_cadence_exact": bool(len(native_times) == EXPECTED_FRAMES and np.allclose(np.diff(native_times), TOUT, rtol=0.0, atol=CADENCE_TOLERANCE_S)),
    ...
    receipt = {
        "schema": "core.f6.physical_anchor.protected_solver_canary_receipt.v1",
        "receipt_id": "F6_explicit_body_v9_protected_solver_canary_receipt_20260921",
