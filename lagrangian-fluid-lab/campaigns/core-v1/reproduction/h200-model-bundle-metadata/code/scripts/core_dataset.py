...
def known_inputs_from_dict(payload):
    return KnownInputs(FiniteGeometry(**payload["geometry"]), PrescribedControl(**payload["control"]),
...
def _load_npz_asset(root, reference, *, kind):
    ...
    try:
        with np.load(path, allow_pickle=False) as archive:
            ...
            if kind == "control":
                return PrescribedControl(archive["samples"])
...
class CoreDataset:
    ...
    def read_state(self, case_id, frame):
        ...
        if case_id not in self._mass_reference:
            self._mass_reference[case_id] = handle["mass"][:] if handle["mass"].ndim == 1 else handle["mass"][0]
        initial_mass = self._mass_reference[case_id]
        if not np.array_equal(mass, initial_mass):
            raise ValueError("changed particle masses require another lifecycle contract")
        state = State(float(handle["time"][frame]), handle["position"][frame], handle["velocity"][frame],
                      handle["particle_id"][:], handle["particle_zone"][:], mass, handle["valid"][frame])
