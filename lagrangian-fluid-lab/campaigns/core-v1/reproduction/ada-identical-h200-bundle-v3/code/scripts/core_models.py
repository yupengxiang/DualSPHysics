...
def two_hop_halo(centers, neighbors, n=None):
    ...
    if torch.is_tensor(neighbors):
        total = int(neighbors.shape[0]) if n is None else int(n)
        centers = torch.as_tensor(centers, dtype=torch.long, device=neighbors.device).reshape(-1)
        s1 = torch.unique(torch.cat((centers, neighbors[centers].reshape(-1))))
        s1 = s1[s1 >= 0]
        s2 = torch.unique(torch.cat((s1, neighbors[s1].reshape(-1))))
        return s1[s1 < total], s2[s2 < total]
    neighbors = np.asarray(neighbors, dtype=np.int64)
    total = neighbors.shape[0] if n is None else int(n)
    ...
    return s1[s1 >= 0], s2[(s2 >= 0) & (s2 < total)]
...
def nearest_geometry(position, geometry):
    ...
    for triangle, wall_v in zip(geometry.triangles, geometry.wall_velocity):
...
def node_features(state, known, dt):
    ...
    rel, normal, wall_v = nearest_geometry(state.position, known.geometry)
...
def tensors(state, known, dt, device):
    """Build full-field torch inputs and the analytic known-force prior."""
