...
用户随后明确补充：即使 GPU 4--7 被长期外部任务占用，也可在显存有安全余量时共跑 L1。执行器每次启动前重新核验 UUID 和显存，至少保留 6144 MiB 空闲显存；运行中每秒复核，低于 4096 MiB（4 GiB）立即终止 L1 solver，并将 L1 solver 限制为单进程串行；GPU 0--3 仍受保护，不终止外来进程。最多 2 个 CPU 重任务，每个默认 8 线程；磁盘至少保留 100 GiB 且不低于容量的 10%。不正式发布、不生成 hidden test、不强推、不合并主线。详见 `L1-RESOURCE-AMENDMENT.md`。
