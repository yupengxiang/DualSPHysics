...
- 数据以完整仿真为划分原子，所有帧、窗口、粒子子集、分辨率复本和派生示踪继承 `lineage_group_id` 的 split。
...
`w10-protocol-adversarial.json` 证明门禁会拒绝：非单调时间、valid 下 NaN、非法刚体变换、同 lineage 跨 split、幸存质量重新归一化、撞击事件采样不足、用 NaN 隐藏预测，以及逐帧身份置换。最后一项通过位置—速度中点积分残差检出，与 W02 的真实身份审计使用同一物理原则。
