...
- 复用原始 601 帧，**没有重跑相同官方 CFD**。新增本地 `campaigns/l1-resume/data/q1-native.h5`，大小 14,049,887,076 bytes，SHA256 `7e69bb52c489c450c8179764f850a948bafeca7b4242c773a888559869798967`。
...
- 601 帧身份唯一、有效字段有限；固定边界位置未变。最终缺失 657 个身份，与 PartOut 的 657 个唯一身份完全一致，均为原生位置排除；没有身份重现。
- 质量 652.212031 → 651.555031 kg，保留率 0.99899266。558 帧超有限壁面容差；最大瞬时壁面异常质量 0.460000 kg。**整体粒子轨迹质量失败。**
...
- H1/H2/H3/H4 水位 RMSE：0.15320/0.03670/0.09315/0.04670 m。H2/H4 仅保留既有 W05 的有限宏观参照范围；H1/H3 存在显著不匹配。**没有新造事后通过阈值，也没有把有限水位用途升级成完整物理正控或 Gold。**
...
- 旧 Q2 累计唯一超容差身份 33 个、累计质量 0.033000 kg；其中底壁 14、右壁 19。最大深度 0.02017467 m，发生在右壁，约 2.017 dp。最大瞬时质量仍为 0.032000 kg。
...
|---|---|---:|---:|---:|
| Q2C3_OFFICIAL_CANONICAL_t06 | quality_failed | 60.060003 | 59.670003 | 0.100000 |
| Q2C4_OFFICIAL_HALFGRID_t06 | quality_failed | 58.905003 | 58.493003 | 0.019000 |
| F1_OFFICIAL_NS_NOP0_dp02_t15 | quality_failed | 635.040030 | 631.824030 | 3.128000 |
...
已使用六个 `_INPUTFIX` 实际 HDF5 产物完成预登记的 21 个观测时刻、三维直方图、TV、质心、q90 和平均速度比较，结果见 `F3-SPATIAL-RESULTS.json`。评分入口已改为显式消费修复后产物映射，不再把物理案例 ID 隐式当作 HDF5 文件名。

plain 粗—中配对的最大指标为 TV=0.14074（阈值 0.05，未通过）、质心=0.00852、q90=0.01243、平均速度=0.03236；后三项通过。plain 中—细及 baffled 配对的完整逐时结果均已写入 JSON。该结果支持 plain 中/细档的完整性进展，但尚不足以宣布合格配方或启动试产。

本 checkpoint 的 CFD、专项评分、状态账本和交接证据均已完成；没有新增 F1/F6，也没有重跑已完成的六例。后续是否进入 plain 专项参考资格和条件式试产，由 reviewer 根据这些真实结果决定。
